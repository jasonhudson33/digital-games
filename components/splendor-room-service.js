...
const channelName = "splendor-room-updates";

export const SplendorRoomService = {
  async createCode() {
    const alphabet = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
    for (let attempt = 0; attempt < 20; attempt += 1) {
      const code = Array.from({ length: 5 }, () => alphabet[Math.floor(Math.random() * alphabet.length)]).join("");
      if (!(await SplendorRoomService.load(code))) return code;
    }
  ...
  subscribe(roomCode, handler) {
    return subscribeToRoomState({
      ...
      channelPrefix: "splendor",
      broadcastName: channelName,
      load: (code) => SplendorRoomService.load(code),
      migrate: (state) => state,
      handler,
      pollInterval: 1200,
    });
...
function loadCachedRoom(code) {
  const cached = localStorage.getItem(`splendor:${code}`);
  return cached ? JSON.parse(cached) : null;
}
function cacheRoom(state) {
  localStorage.setItem(`splendor:${state.roomCode}`, JSON.stringify(state));
...
async function saveRoom(state, expectedUpdatedAt) {
  const response = await fetch(`/api/splendor/rooms/${state.roomCode}`, {
    method: "PUT",
  ...
  if (!response.ok) throw new Error(payload.error || "Could not save the Splendor room.");
  return payload.state;
}
async function loadRoom(code) {
  try {
    const response = await fetch(`/api/splendor/rooms/${code}`, { cache: "no-store" });
    const payload = await readJson(response);
    if (!response.ok) throw new Error(payload.error || "Could not load the Splendor room.");
