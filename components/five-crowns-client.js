...
import { MAX_PLAYERS, MIN_PLAYERS, SUIT_SYMBOLS, addComputerPlayer, addPlayer, canGoOutWithDiscard, createLobby, currentPlayer, discardCard, drawCard, isValidMeld, isWild, layDownMeld, removeComputerPlayer, runComputerStep, startGame, startNextRound, takeDiscard, wildLabel } from "../lib/five-crowns";
import { useGameRoom } from "../lib/use-game-room.js";

export default function FiveCrownsClient() {
  const { room, setRoom, playerId, name, setName, joinCode, setJoinCode, busy, error, createRoom, joinRoom, update } = useGameRoom({ service: FiveCrownsRoomService, storageKey: "five-crowns", createLobby, addPlayer, maxPlayers: MAX_PLAYERS });
...
function Card({ card, wild }) { if (!card) return null; return <span className={`fc-card ${wild ? "wild" : ""} ${card.type === "joker" ? "joker" : card.suit}`}><small>{card.type === "joker" ? "★" : card.rank}</small><strong>{card.type === "joker" ? "JOKER" : SUIT_SYMBOLS[card.suit]}</strong><b>{card.type === "joker" ? "WILD" : card.rank === 11 ? "J" : card.rank === 12 ? "Q" : card.rank === 13 ? "K" : card.rank}</b></span>; }
