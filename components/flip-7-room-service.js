...
const channelName = "flip-7-room-updates";

export const Flip7RoomService = {
  async createCode() {
    const alphabet = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
    for (let attempt = 0; attempt < 20; attempt += 1) {
      const code = Array.from({ length: 5 }, () => alphabet[Math.floor(Math.random() * alphabet.length)]).join("");
      if (!(await Flip7RoomService.load(code))) return code;
    }
  ...
  subscribe(roomCode, handler) {
    return subscribeToRoomState({
      ...
      channelPrefix: "flip-7",
      broadcastName: channelName,
      load: (code) => Flip7RoomService.load(code),
      migrate: (state) => state,
      handler,
      pollInterval: 1200,
    });
...
function loadCachedRoom(code) {
  const cached = localStorage.getItem(`flip-7:${code}`);
  return cached ? JSON.parse(cached) : null;
}

function cacheRoom(state) {
  localStorage.setItem(`flip-7:${state.roomCode}`, JSON.stringify(state));
...
async function saveRoom(state, expectedUpdatedAt) {
  const response = await fetch(`/api/flip-7/rooms/${state.roomCode}`, {
    method: "PUT",
  ...
  if (!response.ok) throw new Error(payload.error || "Could not save the Flip 7 room.");
  return payload.state;
}

async function loadRoom(code) {
  try {
    const response = await fetch(`/api/flip-7/rooms/${code}`, { cache: "no-store" });
    const payload = await readJson(response);
    if (!response.ok) throw new Error(payload.error || "Could not load the Flip 7 room.");
