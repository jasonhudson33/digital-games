...
const channelName = "ticket-to-ride-room-updates";

export const TicketToRideRoomService = {
  async createCode() {
    const alphabet = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
    for (let attempt = 0; attempt < 20; attempt += 1) {
      const code = Array.from({ length: 5 }, () => alphabet[Math.floor(Math.random() * alphabet.length)]).join("");
      if (!(await TicketToRideRoomService.load(code))) return code;
    }
  ...
  subscribe(roomCode, handler) {
    return subscribeToRoomState({
      ...
      channelPrefix: "ticket-to-ride",
      broadcastName: channelName,
      load: (code) => TicketToRideRoomService.load(code),
      migrate: (state) => state,
      handler,
      pollInterval: 1200,
    });
...
function loadCachedRoom(code) {
  const cached = localStorage.getItem(`ticket-to-ride:${code}`);
  return cached ? JSON.parse(cached) : null;
}
function cacheRoom(state) {
  localStorage.setItem(`ticket-to-ride:${state.roomCode}`, JSON.stringify(state));
...
async function saveRoom(state, expectedUpdatedAt) {
  const response = await fetch(`/api/ticket-to-ride/rooms/${state.roomCode}`, {
    method: "PUT",
  ...
  if (!response.ok) throw new Error(payload.error || "Could not save the Ticket to Ride room.");
  return payload.state;
}
async function loadRoom(code) {
  try {
    const response = await fetch(`/api/ticket-to-ride/rooms/${code}`, { cache: "no-store" });
    const payload = await readJson(response);
    if (!response.ok) throw new Error(payload.error || "Could not load the Ticket to Ride room.");
