...
const channelName = "no-thanks-room-updates";

export const NoThanksRoomService = {
  async createCode() {
    ...
    for (let attempt = 0; attempt < 20; attempt += 1) {
      ...
      if (!(await NoThanksRoomService.load(code))) return code;
    }
  ...
  subscribe(roomCode, handler) {
    return subscribeToRoomState({
      ...
      channelPrefix: "no-thanks",
      broadcastName: channelName,
      load: (code) => NoThanksRoomService.load(code),
      migrate: (state) => state,
      handler,
      pollInterval: 1200,
    });
...
function loadCachedRoom(code) {
  const cached = localStorage.getItem(`no-thanks:${code}`);
  return cached ? JSON.parse(cached) : null;
}

function cacheRoom(state) {
  localStorage.setItem(`no-thanks:${state.roomCode}`, JSON.stringify(state));
...
async function saveRoom(state, expectedUpdatedAt) {
  const response = await fetch(`/api/no-thanks/rooms/${state.roomCode}`, {
    method: "PUT",
  ...
  if (!response.ok) throw new Error(payload.error || "Could not save the No Thanks! room.");
  return payload.state;
}

async function loadRoom(code) {
  try {
    const response = await fetch(`/api/no-thanks/rooms/${code}`, { cache: "no-store" });
    const payload = await readJson(response);
    if (!response.ok) throw new Error(payload.error || "Could not load the No Thanks! room.");
