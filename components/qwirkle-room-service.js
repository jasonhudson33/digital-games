...
const channelName = "qwirkle-room-updates";

export const QwirkleRoomService = {
  async createCode() {
    const alphabet = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789";
    for (let attempt = 0; attempt < 20; attempt += 1) {
      const code = Array.from({ length: 5 }, () => alphabet[Math.floor(Math.random() * alphabet.length)]).join("");
      if (!(await QwirkleRoomService.load(code))) return code;
    }
  ...
  async update(roomCode, updater) {
    ...
    throw new Error("The room changed while this turn was being saved. Please try again.");
  },
  ...
  subscribe(roomCode, handler) {
    return subscribeToRoomState({
      ...
      channelPrefix: "qwirkle",
      broadcastName: channelName,
      load: (code) => QwirkleRoomService.load(code),
      migrate: (state) => state,
      handler,
      pollInterval: 1200,
    });
...
function loadCachedRoom(code) {
  const cached = localStorage.getItem(`qwirkle:${code}`);
  return cached ? JSON.parse(cached) : null;
}
function cacheRoom(state) {
  localStorage.setItem(`qwirkle:${state.roomCode}`, JSON.stringify(state));
...
async function saveRoom(state, expectedUpdatedAt) {
  const response = await fetch(`/api/qwirkle/rooms/${state.roomCode}`, {
    method: "PUT",
  ...
  if (!response.ok) throw new Error(payload.error || "Could not save the Qwirkle room.");
  return payload.state;
}
async function loadRoom(code) {
  try {
    const response = await fetch(`/api/qwirkle/rooms/${code}`, { cache: "no-store" });
    const payload = await readJson(response);
    if (!response.ok) throw new Error(payload.error || "Could not load the Qwirkle room.");
