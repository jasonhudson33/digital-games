...
import { bankTotal, getKaballasMarket, getKillerBunniesCardPlayStatus, getKillerBunniesCloverReduction, getKillerBunniesExtraRunStatus, getKillerBunniesFeedingStatus, getKillerBunniesPileStatus, getKillerBunniesShopItemStatus } from "../lib/killer-bunnies";
    ...
    <main className="kb-app kb-game-shell">
      ...
      <section className="kb-table" aria-label="Killer Bunnies tabletop">
        ...
        <div className="kb-felt-center">
          <div className={`kb-turn-message phase-${game.phase}`}><span>{game.phase === "setupRun" ? isOpeningTurn ? "PROGRAM YOUR RUN" : "OPENING SETUP" : mustDefend || mustResolveImmediate || mustPlaceModifier || mustResolveDefector ? "ACTION NEEDED" : yourTurn ? "YOUR TURN" : `${game.players[game.currentPlayerIndex]?.name?.toUpperCase() || "GAME"}`}</span><p>{game.message}</p>{game.lastRoll && <b>d{game.lastRoll.sides}: {game.lastRoll.value}</b>}</div>
      ...
      {mustResolveImmediate && <ImmediateCardDialog pending={game.pendingAction} player={viewer} busy={busy} onConfirm={() => onAction("resolveImmediateCard")} onTarget={(bunnyId) => onAction("chooseMisfortuneTarget", { bunnyId })} />}
