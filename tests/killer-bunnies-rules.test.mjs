...
test("the prototype creates exactly 181 cards including Kaballa's Market starter card", () => {
  ...
  assert.equal(game.mainDeck.length + mainCardsInPlayerZones, 108);
  assert.equal(game.carrotMarket.length, 12);
  assert.equal(game.magicCarrotDeck.length, 12);
  assert.equal(game.cabbageSupply.length, 24);
  assert.equal(game.waterSupply.length, 24);
  assert.equal(game.kaballasMarket.name, "Kaballa’s Market");
...
test("market RUN cards can close the store while Choose A Carrot still works", () => {
  ...
  const closedCardIndex = game.mainDeck.findIndex((card) => card.id === "market-closed-1");
  const [closedCard] = game.mainDeck.splice(closedCardIndex, 1);
...
test("all eighteen expansion packs are independently modular and change exact card counts", () => {
  assert.equal(KILLER_BUNNIES_EXPANSIONS.length, 18);
  assert.equal(new Set(KILLER_BUNNIES_EXPANSIONS.map((pack) => pack.id)).size, 18);
  assert.equal(KILLER_BUNNIES_EXPANSIONS.find((pack) => pack.id === "red").cardCounts.total, 25);
  assert.equal(KILLER_BUNNIES_EXPANSIONS.find((pack) => pack.id === "orange").cardCounts.total, 19);
  assert.ok(KILLER_BUNNIES_EXPANSIONS.filter((pack) => !["red", "orange"].includes(pack.id)).every((pack) => pack.cardCounts.total === 18));
  ...
  assert.equal(content.mainDeck.length, 24);
  assert.deepEqual(game.expansionIds, expansionIds);
  assert.equal(game.expansionSummary.packCount, 2);
  assert.equal(game.cardCounts.total, 224);
  assert.equal(game.cardCounts.mainDeck, 132);
  assert.equal(game.cardCounts.carrotMarket, 14);
  assert.equal(game.cardCounts.magicCarrots, 14);
  assert.equal(game.cardCounts.cabbage, 28);
  assert.equal(game.cardCounts.water, 28);
...
test("Red and Orange boosters add Rooney's Weapons Emporium and Weil's Pawn Shop", () => {
  ...
  assert.equal(game.cardCounts.total, 225);
...
test("a computer-only game can finish with every expansion enabled", () => {
  ...
  assert.equal(game.expansionSummary.packCount, 18);
  assert.equal(game.cardCounts.total, 513);
  assert.equal(game.players.reduce((total, player) => total + player.carrots.length, 0), 30);
});
