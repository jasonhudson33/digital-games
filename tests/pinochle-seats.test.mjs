...
import { PHONE_TABLE, WIDE_TABLE, backFan, clearance, seatLayout } from "../lib/pinochle-seats.js";
import { createPinochleGame } from "../lib/pinochle.js";

/*
 * The seating has to hold for every table Pinochle deals — two through six —
 * and it is geometry, so it can be checked rather than eyeballed. These are the
 * invariants that make the table readable: nobody overlaps anybody, partners
 * face each other, and the card you played is nearer to you than to anyone else.
 *
 * Both shapes are checked, because the stylesheet draws both. A phone is not the
 * desktop scaled down — it has its own plate, its own card and a portrait felt —
 * and it was the one that failed while only the desktop was under test.
 */

const COUNTS = [2, 3, 4, 5, 6];
const SHAPES = [["desktop", WIDE_TABLE], ["phone", PHONE_TABLE]];
...
/** Every table Pinochle deals, in both shapes the stylesheet draws. */
function everyTable(run) {
  for (const [name, shape] of SHAPES) {
    for (const count of COUNTS) run(seatLayout(count, 0, shape), shape, `${name} ${count}-handed`);
  }
}
  ...
  for (const [name, shape] of SHAPES) {
    for (const count of COUNTS) {
      for (let viewer = 0; viewer < count; viewer += 1) {
        ...
        assert.equal(mine.relative, 0, `${name} ${count}-handed, seat ${viewer}`);
...
test("bid and meld pills hang away from the middle of the table", () => {
