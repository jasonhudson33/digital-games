...
export function createKillerBunniesGame({ playerSeeds, expansionIds = [], random = Math.random }) {
  ...
  const game = {
    ...
    zodiacDeck: selectedExpansionIds.includes("green") ? shuffle(ZODIAC_SIGNS.map((sign, index) => ({ id: `zodiac-small-${index}`, sign })), random) : [],
    winningZodiac: null,
    barriers: [],
    futureBunnies: [],
    winnerIndexes: [],
  ...
  for (const [playerIndex, player] of game.players.entries()) {
    while (player.hand.length < 7) dealPlayableCard(game, player, playerIndex, random);
  }
...
export function playTopRun(game, playerIndex, random = Math.random) {
  ...
  if (!card) throw new Error(`Your TOP RUN slot is empty (${player.name}, turn ${next.turnNumber}, last: ${next.log?.[0] || "none"}).`);
...
export function resolveKillerBunniesSpecialChoice(game, playerIndex, choice, random = Math.random) {
  ...
  if (choice === "save") {
    ...
    if (Number(nextPending.card.number) === 362) {
      next.phase = "albinoTarget";
      next.pendingAction = { playerIndex, cardPlayerIndex: playerIndex, effect: "albinoBaby", card: nextPending.card };
      next.message = "Choose the player whose bunnies Albino Baby 5000 will protect.";
      return next;
    }
...
export function playSavedKillerBunniesSpecial(game, playerIndex, cardId, random = Math.random) {
  ...
  if (Number(playedCard.number) === 258) return cancelWithAntiMissile(next, playerIndex, playedCard);
  if (Number(playedCard.number) === 317) return cancelVerySpecialWithUriel(next, playerIndex, playedCard);
  if (Number(playedCard.number) === 318) return cancelFreshnessWithWeilerware(next, playerIndex, playedCard);
  if (Number(playedCard.number) === 363) {
    const event = next.lastBunnyDeathEvent;
    next.phase = "spoilsportTarget";
    next.pendingAction = {
      playerIndex, effect: "operationSpoilsport", card: playedCard,
      targetPlayerIndex: event.killerPlayerIndex, maxKills: event.count,
      returnPhase: game.phase, resumePlayerIndex,
    };
    next.message = `Choose ${Math.min(event.count, next.players[event.killerPlayerIndex].bunnies.length)} of ${next.players[event.killerPlayerIndex].name}'s bunnies for Operation Spoilsport.`;
    return next;
  }
  if (Number(playedCard.number) === 364) {
    next.players[playerIndex].nextD12UsesD20 = true;
    next.discardPile.push(playedCard);
    pushLog(next, `${next.players[playerIndex].name} upgraded their next numeric d12 roll to the Clear d20.`);
    return next;
  }
...
export function getKillerBunniesSavedSpecialStatus(game, playerIndex, card) {
  ...
  if (Number(card.number) === 258) {
    return canAntiMissileCancel(game, playerIndex)
      ? { enabled: true, reason: `Eliminate ${game.pendingAction.card.name}.` }
      : { enabled: false, reason: "Anti-Missile needs a listed Weapon currently attacking one of your bunnies." };
  }
  if (Number(card.number) === 317) {
    return game.pendingAction?.card?.type === "VERY SPECIAL" && game.pendingAction.playerIndex !== playerIndex
      ? { enabled: true, reason: `Cancel ${game.pendingAction.card.name}.` }
      : { enabled: false, reason: "Uriel's Machine needs an opponent's Very Special awaiting resolution." };
  }
  if (Number(card.number) === 318) {
    return game.phase === "freshnessChoice" && game.pendingAction?.targetPlayerIndex === playerIndex
      ? { enabled: true, reason: "Cancel Weil's Freshness Center." }
      : { enabled: false, reason: "Weilerware protects you while Weil's Freshness Center is resolving." };
  }
  if (Number(card.number) === 362) {
    const protectedPlayer = game.players[card.protectedPlayerIndex ?? playerIndex];
    return { enabled: false, reason: `Albino Baby 5000 is passively protecting ${protectedPlayer?.name || "its assigned player"}.` };
  }
  if (Number(card.number) === 363) {
    const event = game.lastBunnyDeathEvent;
    return event?.victimPlayerIndex === playerIndex && event.killerPlayerIndex !== playerIndex && event.turnNumber === game.turnNumber
      ? { enabled: true, reason: `Eliminate up to ${event.count} of ${game.players[event.killerPlayerIndex]?.name}'s bunnies.` }
      : { enabled: false, reason: "Operation Spoilsport needs an opponent-caused loss of one of your bunnies this turn." };
  }
  if (Number(card.number) === 364) {
    const isNumericD12Roll = ["defend", "areaWeaponRoll", "cardDiceRoll", "blueSpecialRoll", "blueCardRoll", "roamingRoll", "defectorRoll", "povertyPokerRoll", "dayDeadRoll", "precessionBirth"].includes(game.phase);
    return isNumericD12Roll && game.pendingAction?.playerIndex === playerIndex
      ? { enabled: true, reason: "Upgrade your next numeric d12 roll to the Clear d20." }
      : { enabled: false, reason: "Roll-O-Lution needs a numeric d12 roll you are about to make." };
  }
...
function resolveTimidRerollChoice(game, playerIndex, choice, random) {
  ...
  original.rollSequence = Math.max(original.rollSequence || 0, pending.acceptedState?.rollSequence || 0);
...
function resolveKillerBunniesCardActionCore(game, playerIndex, action = {}, random = Math.random) {
  ...
  switch (next.phase) {
    ...
    case "lowJackRoll": return resolveLowJackKojak(next, pending, random);
    case "bunnyHop": return resolveBunnyHop(next, pending, Number(action.leftPlayerIndex));
    case "coolChange": return resolveCoolChange(next, pending, action.cardIds || []);
    case "laHotPeppers": return resolveLaHotPeppers(next, pending, action.obligationId);
    case "paintballTarget": return resolveRandomPaintball(next, pending, action, random);
    case "runAmokTarget": return resolveRunAmok(next, pending, Number(action.targetPlayerIndex));
    case "runTransformer": return resolveRunTransformer(next, pending, action.cardId, random);
    case "pilferPawnRoll": return resolvePilferPawn(next, pending, random);
    case "freePawnChoice": return resolveFreePawn(next, pending, action.pawnIds || []);
    case "randomFeedRoll": return resolveRandomFeedRoll(next, pending, random);
    case "royaleGamble": return resolveCarrotTopRoyaleGamble(next, pending, action.choice, random);
    case "f18Crew": return resolveF18Crew(next, pending, action.bunnyIds || []);
    case "dayDeadRoll": return resolveDayOfTheDeadRoll(next, pending, random);
    case "dayDeadRevive": return resolveDayOfTheDeadRevive(next, pending, action.bunnyIds || []);
    case "fingercuffs": return resolveFingercuffs(next, pending, action.bunnyIds || []);
    case "precessionBirth": return resolvePrecession(next, pending, action.birthZodiac, random);
    case "laTapeWorm": return resolveLaTapeWorm(next, pending, action.obligationId);
    case "zodiacResetRun": return resolveZodiacResetRun(next, pending, action.cardIds || []);
    case "leifTarget": return resolveLeifCarrotson(next, pending, Number(action.targetPlayerIndex), random);
    case "spoilsportTarget": return resolveOperationSpoilsport(next, pending, action.bunnyIds || []);
    case "albinoTarget": return resolveAlbinoAssignment(next, pending, Number(action.targetPlayerIndex));
    case "zodiacPrivilege": return resolveWinningZodiacPrivilege(next, pending, action);
    case "leifRoll": return resolveLeifCarrotsonRoll(next, pending, random);
    case "leifPassenger": return resolveLeifCarrotsonPassenger(next, pending, action.bunnyId);
...
function resolveKillerBunniesCardDiceRollCore(game, playerIndex, choiceId, random = Math.random) {
  ...
  recordDiceRoll(next, results, pending.card.name);
...
export function chooseKillerBunniesMisfortuneTarget(game, playerIndex, bunnyId, targetPlayerIndex = playerIndex) {
  ...
  if (Number(pending.card.number) === 85 && hasAlbinoProtection(next, targetPlayerIndex, { number: 294 })) {
    next.discardPile.push(pending.card);
    next.pendingAction = null;
    pushLog(next, `${target.name}'s Albino Baby 5000 blocked ${pending.card.name}.`);
    return finishImmediateEffect(next, pending);
  }
...
export function chooseKillerBunniesTarget(game, playerIndex, targetPlayerIndex, bunnyId, random = Math.random) {
  ...
  if (pending.effect === "feed" && pending.card.randomCost) {
    ...
    next.message = `${target.name}: roll the Green and Blue d12s to set ${pending.card.name}'s Cabbage and Water costs.`;
    return next;
  }
  ...
  if (pending.effect === "weapon" && hasAlbinoProtection(next, targetPlayerIndex, pending.card)) {
    pushLog(next, `${target.name}'s Albino Baby 5000 blocked ${pending.card.name}.`);
    return finishUsedWeapon(next, pending, `${target.name}'s Albino Baby 5000 blocked ${pending.card.name}.`);
  }
  ...
  next.phase = "defend";
  const cloverReduction = pending.effect === "weapon" ? getEffectiveCloverReduction(next, bunny) : 0;
  ...
  next.message = pending.effect === "weapon" && cloverReduction > 0
    ...
    : `${target.name} must roll a d${Number(pending.card.power) >= 13 ? 20 : 12} for ${bunny.name}.`;
...
function resolveKillerBunniesAreaWeaponRollCore(game, playerIndex, random = Math.random) {
  ...
  const cloverReduction = getEffectiveCloverReduction(next, circleEntry.bunny);
  ...
  recordDiceRoll(next, [{ value, sides, color: sides === 20 ? "clear" : null }], pending.card.name);
  pending.rollQueue.shift();

  if (value <= effectivePower) {
    recordOpponentBunnyKill(next, circleEntry.playerIndex, pending.attackingPlayerIndex);
...
export function chooseKillerBunniesPlayerTarget(game, playerIndex, targetPlayerIndex, random = Math.random) {
  ...
  if (number === 51) {
    ...
    const portfolioIndex = target.savedSpecials?.findIndex((card) => Number(card.number) === 251) ?? -1;
    if (portfolioIndex >= 0) {
      const [portfolio] = target.savedSpecials.splice(portfolioIndex, 1);
      next.discardPile.push(portfolio);
      pushLog(next, `${target.name}'s ${portfolio.name} prevented Bankruptcy.`);
    } else {
    ...
    if (targetPlayerIndex === playerIndex) throw new Error("Choose an opponent for Mothership.");
    const protectedIndex = target.savedSpecials?.findIndex((card) => Number(card.number) === 259) ?? -1;
    if (protectedIndex >= 0) {
      const [protection] = target.savedSpecials.splice(protectedIndex, 1);
      next.discardPile.push(protection);
      pushLog(next, `${target.name}'s ${protection.name} prevented ${pending.card.name}.`);
    } else {
      restoreArea51Bunny(next);
      const abducted = target.bunnies.filter((bunny) => !hasHeavenlyHalo(bunny));
      target.bunnies = target.bunnies.filter((bunny) => hasHeavenlyHalo(bunny));
      for (const bunny of abducted) removeKillerBunnyFromCircle(next, bunny.id);
      next.area51Abducted = { bunnies: abducted, ownerIndex: targetPlayerIndex };
      pushLog(next, `${pending.card.name} abducted all ${abducted.length} of ${target.name}'s bunnies.`);
    }
  } else {
...
export function chooseKillerBunniesUtilityBunnyTarget(game, playerIndex, targetPlayerIndex, bunnyId) {
  ...
  if ([49, 50, 134].includes(number)) {
    const protectionIndex = target.savedSpecials?.findIndex((card) => Number(card.number) === 259) ?? -1;
    if (protectionIndex >= 0) {
      const [protection] = target.savedSpecials.splice(protectionIndex, 1);
      next.discardPile.push(protection, pending.card);
      next.pendingAction = null;
      pushLog(next, `${target.name}'s ${protection.name} prevented ${pending.card.name}.`);
      return finishPendingEffect(next, pending);
    }
    if (hasHeavenlyHalo(bunny)) {
      const message = `${bunny.name}'s Heavenly Halo blocked ${pending.card.name}.`;
      pushLog(next, message);
      next.discardPile.push(pending.card);
      next.pendingAction = null;
      const continued = finishPendingEffect(next, pending);
      continued.message = `${message}${continued.phase === "draw" ? " Click the main draw pile." : ""}`;
      return continued;
    }
    restoreArea51Bunny(next);
  ...
  } else if ([72, 73, 253].includes(number)) {
...
function rollRussianRoulette(game, pending, random) {
  ...
  recordDiceRoll(game, [{ value, sides: 12, color: null }], pending.card.name);
...
function rerollRussianRoulette(game, pending, choice, random) {
  if (choice === "reroll") {
    ...
    recordDiceRoll(game, [{ value, sides: 12, color: null }], `${pending.card.name} reroll`);
...
function resolveFeedAllTarget(game, pending, targetPlayerIndex) {
  ...
  game.discardPile.push(pending.card);
  game.pendingAction = null;
  pushLog(game, `${target.name}'s ${target.bunnies.length} current bunnies must each feed ${pending.card.cabbageCost || 1} Cabbage and ${pending.card.waterCost || 1} Water.`);
  return finishPendingEffect(game, pending);
}

function resolveShareTheWealth(game, playerIndex, card, options) {
  const actor = game.players[playerIndex];
  const opponents = game.players.map((player, index) => ({ player, index, total: bankTotal(player) }))
    .filter((entry) => entry.index !== playerIndex);
  const richestTotal = Math.max(0, ...opponents.map((entry) => entry.total));
  const richest = opponents.find((entry) => entry.total === richestTotal);
  const amount = Math.floor(richestTotal / 2);
  if (bankTotal(actor) === 0 && richest && amount > 0) {
    spendBank(richest.player, amount, game.discardPile);
    awardDollaFromDiscard(game, actor, amount);
    pushLog(game, `${actor.name} took ${amount} Dolla from ${richest.player.name} with ${card.name}.`);
  } else {
    pushLog(game, `${card.name} transferred no Dolla.`);
  }
  game.discardPile.push(card);
  return finishCardEffect(game, options);
}

function resolveBunniesInBlack(game, playerIndex, card, options) {
  const actor = game.players[playerIndex];
  const abducted = game.area51Abducted;
  const bunnies = abducted?.bunnies || (abducted?.bunny ? [abducted.bunny] : []);
  if (bunnies.length) {
    actor.bunnies.push(...bunnies);
    for (const bunny of bunnies) addKillerBunnyToCircle(game, bunny.id);
    game.area51Abducted = null;
    pushLog(game, `${actor.name} recovered ${bunnies.map((bunny) => bunny.name).join(" and ")} with ${card.name}.`);
  } else {
    actor.savedSpecials ||= [];
    actor.savedSpecials.push(card);
    pushLog(game, `${actor.name} saved ${card.name} as protection against the next abduction.`);
    return finishCardEffect(game, options);
  }
  game.discardPile.push(card);
  return finishCardEffect(game, options);
...
function resolveLowJackKojak(game, pending, random) {
  const roll = rollDie(20, random);
  recordDiceRoll(game, [{ value: roll, sides: 20, color: "clear" }], pending.card.name);
  const actor = game.players[pending.playerIndex];
  let found = removeFirst(game.carrotMarket, (carrot) => Number(carrot.label || carrot.carrotKey) === roll);
  let source = "Kaballa's Market";
  if (!found) {
    const ownerIndex = game.players.findIndex((player) => player.carrots.some((carrot) => Number(carrot.label || carrot.carrotKey) === roll));
    if (ownerIndex >= 0) {
      const owner = game.players[ownerIndex];
      found = removeFirst(owner.carrots, (carrot) => Number(carrot.label || carrot.carrotKey) === roll);
      game.carrotMarket.push(...owner.carrots.splice(0));
      source = owner.name;
    }
  }
  if (found) actor.carrots.push(found);
  game.discardPile.push(pending.card);
  game.pendingAction = null;
  pushLog(game, found ? `${actor.name} rolled ${roll} and took Carrot ${roll} from ${source}.` : `${actor.name} rolled ${roll}; that Carrot was not in play.`);
  return finishPendingEffect(game, pending);
}

function resolveBunnyHop(game, pending, leftPlayerIndex) {
  if (!Number.isInteger(leftPlayerIndex) || leftPlayerIndex < 0 || leftPlayerIndex >= game.players.length) throw new Error("Choose a valid adjacent-player boundary.");
  const rightPlayerIndex = (leftPlayerIndex + 1) % game.players.length;
  const left = game.players[leftPlayerIndex];
  const right = game.players[rightPlayerIndex];
  if (left.topRun && right.topRun) [left.topRun, right.topRun] = [right.topRun, left.topRun];
  if (left.bottomRun && right.bottomRun) [left.bottomRun, right.bottomRun] = [right.bottomRun, left.bottomRun];
  game.discardPile.push(pending.card);
  game.pendingAction = null;
  pushLog(game, `${left.name} and ${right.name} exchanged both programmed RUN cards.`);
  return finishPendingEffect(game, pending);
}

function resolveCoolChange(game, pending, cardIds) {
  const actor = game.players[pending.playerIndex];
  const chosen = new Set(cardIds);
  const slots = [];
  actor.hand.forEach((card, index) => { if (chosen.has(card.id)) slots.push({ zone: "hand", index, card }); });
  if (actor.topRun && chosen.has(actor.topRun.id)) slots.push({ zone: "topRun", card: actor.topRun });
  if (actor.bottomRun && chosen.has(actor.bottomRun.id)) slots.push({ zone: "bottomRun", card: actor.bottomRun });
  if (slots.length !== chosen.size) throw new Error("Choose only cards from your hand or RUN slots.");
  actor.hand = actor.hand.filter((card) => !chosen.has(card.id));
  if (actor.topRun && chosen.has(actor.topRun.id)) actor.topRun = null;
  if (actor.bottomRun && chosen.has(actor.bottomRun.id)) actor.bottomRun = null;
  game.discardPile.push(...slots.map((slot) => slot.card), pending.card);
  for (const slot of slots) {
    let replacement = drawMain(game);
    while (replacement && (replacement.kind === "money" || isPlayImmediately(replacement))) {
      if (replacement.kind === "money") actor.bank.push(replacement);
      else game.immediateQueue.push({ playerIndex: pending.playerIndex, card: replacement });
      replacement = drawMain(game);
    }
    if (!replacement) continue;
    if (slot.zone === "hand") actor.hand.push(replacement);
    else actor[slot.zone] = replacement;
  }
  game.pendingAction = null;
  pushLog(game, `${actor.name} replaced ${slots.length} card${slots.length === 1 ? "" : "s"} with ${pending.card.name}.`);
  if (game.immediateQueue.length) return startNextQueuedImmediate(game, effectReturnState(pending));
  return finishPendingEffect(game, pending);
}

function resolveLaHotPeppers(game, pending, obligationId) {
  const target = game.players.find((player) => (player.feedingObligations || []).some((entry) => entry.id === obligationId));
  const obligation = target?.feedingObligations.find((entry) => entry.id === obligationId);
  if (!obligation) throw new Error("Choose a pending Feed the Bunny card.");
  obligation.waterCost *= 2;
  game.discardPile.push(pending.card);
  game.pendingAction = null;
  pushLog(game, `${pending.card.name} doubled ${target.name}'s Water requirement to ${obligation.waterCost}.`);
  return finishPendingEffect(game, pending);
}

function resolveRandomPaintball(game, pending, action, random) {
  const targetPlayerIndex = Number(action.targetPlayerIndex);
  const owner = game.players[targetPlayerIndex];
  const collection = action.itemType === "pawn" ? owner?.pawns : owner?.bunnies;
  const item = collection?.find((entry) => entry.id === action.itemId);
  if (!item) throw new Error("Choose a bunny or Pawn in play.");
  const colors = ["violet", "orange", "green", "yellow", "blue", "black", "red"];
  const dice = colors.map((color) => ({ color, sides: 12, value: rollDie(12, random) }));
  recordDiceRoll(game, dice, pending.card.name, owner.name);
  const high = Math.max(...dice.map((die) => die.value));
  const winners = dice.filter((die) => die.value === high).map((die) => die.color);
  if (action.itemType !== "pawn" && winners.includes("black")) eliminateCircleEntry(game, findKillerBunnyInCircle(game, item.id), `${pending.card.name}'s Black high roll`);
  else {
    item.colors = winners;
    item.color = winners[0];
  }
  game.discardPile.push(pending.card);
  game.pendingAction = null;
  pushLog(game, `${pending.card.name} rolled ${winners.join("/")} high (${high}) for ${item.name}.`);
  return finishPendingEffect(game, pending);
}

function resolveRunAmok(game, pending, targetPlayerIndex) {
  const target = game.players[targetPlayerIndex];
  if (!target || targetPlayerIndex === pending.playerIndex) throw new Error("Choose an opponent.");
  const oldTop = target.topRun;
  const oldBottom = target.bottomRun;
  target.topRun = drawProgrammedReplacement(game, target, targetPlayerIndex) || oldTop;
  target.bottomRun = drawProgrammedReplacement(game, target, targetPlayerIndex) || oldBottom;
  if (oldTop && target.topRun !== oldTop) game.discardPile.push(oldTop);
  if (oldBottom && target.bottomRun !== oldBottom) game.discardPile.push(oldBottom);
  game.discardPile.push(pending.card);
  game.pendingAction = null;
  pushLog(game, `${target.name}'s TOP and BOTTOM RUN cards were replaced by ${pending.card.name}.`);
  return finishPendingEffect(game, pending);
}

function drawProgrammedReplacement(game, player, playerIndex) {
  let card = drawMain(game);
  while (card && (card.kind === "money" || isPlayImmediately(card))) {
    if (card.kind === "money") player.bank.push(card);
    else game.immediateQueue.push({ playerIndex, card });
    card = drawMain(game);
  }
  return card || null;
}

function resolveRunTransformer(game, pending, cardId, random) {
  const player = game.players[pending.playerIndex];
  const index = player.hand.findIndex((card) => card.id === cardId && card.type === "RUN");
  if (index < 0 || !pending.eligibleIds.includes(cardId)) throw new Error("Choose a RUN card from your hand.");
  const [transformed] = player.hand.splice(index, 1);
  game.discardPile.push(pending.card);
  game.pendingAction = null;
  pushLog(game, `${player.name} transformed ${transformed.name} into an immediate play.`);
  return resolveRunCard(game, pending.playerIndex, transformed, random, pending);
}

function resolvePilferPawn(game, pending, random) {
  const colors = ["violet", "orange", "green", "yellow", "blue", "black", "red"];
  const dice = colors.map((color) => ({ color, sides: 12, value: rollDie(12, random) }));
  recordDiceRoll(game, dice, pending.card.name);
  const high = Math.max(...dice.map((die) => die.value));
  const winners = dice.filter((die) => die.value === high).map((die) => die.color);
  const actor = game.players[pending.playerIndex];
  const taken = [];
  for (const color of winners) {
    let pawn;
    for (let index = 0; index < game.players.length && !pawn; index += 1) {
      if (index !== pending.playerIndex) pawn = removeFirst(game.players[index].pawns || [], (entry) => entry.color === color);
    }
    pawn ||= removeFirst(game.weilsPawnShop?.pawnSupply || [], (entry) => entry.color === color);
    if (pawn) { actor.pawns.push(pawn); taken.push(pawn); }
  }
  game.discardPile.push(pending.card);
  game.pendingAction = null;
  pushLog(game, `${actor.name} pilfered ${taken.length ? taken.map((pawn) => pawn.name).join(" and ") : "no Pawns"}.`);
  return finishPendingEffect(game, pending);
}

function resolveFreePawn(game, pending, pawnIds) {
  const uniqueIds = [...new Set(pawnIds)];
  if (!uniqueIds.length || uniqueIds.length > 2) throw new Error("Choose one or two Pawns.");
  const supply = game.weilsPawnShop?.pawnSupply || [];
  if (uniqueIds.some((id) => !supply.some((pawn) => pawn.id === id))) throw new Error("Choose available Pawns from Weil's Pawn Shop.");
  const actor = game.players[pending.playerIndex];
  for (const id of uniqueIds) actor.pawns.push(removeFirst(supply, (pawn) => pawn.id === id));
  game.discardPile.push(pending.card);
  game.pendingAction = null;
  pushLog(game, `${actor.name} took ${uniqueIds.length} free Pawn${uniqueIds.length === 1 ? "" : "s"}.`);
  return finishPendingEffect(game, pending);
}

function resolveF18Crew(game, pending, bunnyIds) {
  const uniqueIds = [...new Set(bunnyIds)];
  const actor = game.players[pending.playerIndex];
  if (uniqueIds.length !== 2 || uniqueIds.some((id) => !actor.bunnies.some((bunny) => bunny.id === id))) {
    throw new Error("Choose exactly two of your bunnies to crew the F-18 Hornet.");
  }
  game.phase = "roamingTarget";
  game.pendingAction = { ...pending, effect: "roamingTarget", crewBunnyIds: uniqueIds };
  game.message = `${actor.name}: choose the F-18 Hornet's first opposing bunny target.`;
  return game;
}

function resolveRandomFeedRoll(game, pending, random) {
  const cabbageCost = rollDie(12, random);
  const waterCost = rollDie(12, random);
  recordDiceRoll(game, [
    { value: cabbageCost, sides: 12, color: "green" },
    { value: waterCost, sides: 12, color: "blue" },
  ], pending.card.name);
  const target = game.players[pending.targetPlayerIndex];
  if (!target?.bunnies.some((bunny) => bunny.id === pending.bunnyId)) throw new Error("That bunny is no longer available.");
  if (cabbageCost === waterCost) {
    const bunnyIndex = target.bunnies.findIndex((bunny) => bunny.id === pending.bunnyId);
    if (supplyUnitTotal(target, "cabbage") >= cabbageCost && supplyUnitTotal(target, "water") >= waterCost) {
      spendSupplyUnits(game, target, "cabbage", cabbageCost);
      spendSupplyUnits(game, target, "water", waterCost);
      pushLog(game, `${target.name} rolled doubles and immediately fed ${target.bunnies[bunnyIndex].name}.`);
    } else {
      const [bunny] = target.bunnies.splice(bunnyIndex, 1);
      discardBunny(game, bunny);
      pushLog(game, `${target.name} rolled doubles but could not immediately feed ${bunny.name}.`);
    }
    game.discardPile.push(pending.card);
    game.pendingAction = null;
    return finishPendingEffect(game, pending);
  ...
  target.feedingObligations ||= [];
  target.feedingObligations.push({
    id: `feeding-${pending.card.id}-${pending.bunnyId}`,
    bunnyId: pending.bunnyId,
    card: pending.card,
    attackingPlayerIndex: pending.attackingPlayerIndex,
    cabbageCost,
    waterCost,
  ...
  game.pendingAction = null;
  pushLog(game, `${target.name} must feed ${cabbageCost} Cabbage and ${waterCost} Water by the end of their next turn.`);
  return finishPendingEffect(game, pending);
}

function resolveDayOfTheDeadRoll(game, pending, random) {
  const colors = ["violet", "orange", "green", "yellow", "blue", "red", "pink"];
  const dice = colors.flatMap((color) => Array.from({ length: 3 }, () => ({ value: rollDie(12, random), sides: 12, color })));
  const successfulColors = colors.filter((color) => dice.some((die) => die.color === color && die.value === 6));
  recordDiceRoll(game, dice, pending.card.name);
  const discarded = [
    ...(game.weilsPawnShop?.bunnyDiscard || []),
    ...game.discardPile.filter((card) => card.kind === "bunny"),
  ];
  const eligible = discarded.filter((bunny) => {
    if (/Free Agent|Specialty|Celebrity/i.test(bunny.name || "")) return false;
    return bunnyColors(bunny).some((color) => successfulColors.includes(color));
  });
  if (!eligible.length) {
    game.discardPile.push(pending.card);
    game.pendingAction = null;
    pushLog(game, `${pending.card.name} rolled ${successfulColors.length ? successfulColors.join(", ") : "no successful colors"}, but found no eligible discarded bunny.`);
    return finishPendingEffect(game, pending);
  }
  game.phase = "dayDeadRevive";
  game.pendingAction = { ...pending, successfulColors, eligibleBunnyIds: eligible.map((bunny) => bunny.id), maxRevive: Math.min(3, eligible.length) };
  game.message = `Choose up to ${Math.min(3, eligible.length)} ordinary or half-color bunnies matching ${successfulColors.join(", ")}.`;
  return game;
}

function resolveDayOfTheDeadRevive(game, pending, bunnyIds) {
  const ids = [...new Set(bunnyIds)].slice(0, pending.maxRevive || 3);
  if (!ids.length) throw new Error("Choose at least one eligible bunny to revive.");
  if (ids.some((id) => !pending.eligibleBunnyIds.includes(id))) throw new Error("Choose only eligible discarded bunnies.");
  const player = game.players[pending.cardPlayerIndex ?? pending.playerIndex];
  const revived = [];
  for (const id of ids) {
    let bunny = removeFirst(game.weilsPawnShop?.bunnyDiscard || [], (entry) => entry.id === id);
    bunny ||= removeFirst(game.discardPile, (entry) => entry.id === id && entry.kind === "bunny");
    if (!bunny) continue;
    player.bunnies.push(bunny);
    addKillerBunnyToCircle(game, bunny.id);
    revived.push(bunny);
  }
  game.discardPile.push(pending.card);
  game.pendingAction = null;
  pushLog(game, `${player.name} revived ${revived.map((bunny) => bunny.name).join(", ")} with ${pending.card.name}.`);
  return finishPendingEffect(game, pending);
}

function resolveFingercuffs(game, pending, bunnyIds) {
  const ids = [...new Set(bunnyIds)];
  if (ids.length !== 2) throw new Error("Choose exactly two adjacent bunnies.");
  const circle = getKillerBunniesCircleEntries(game);
  const positions = ids.map((id) => circle.findIndex((entry) => entry.bunny.id === id));
  if (positions.some((index) => index < 0)) throw new Error("Both bunnies must still be in the Bunny Circle.");
  const distance = Math.abs(positions[0] - positions[1]);
  if (distance !== 1 && distance !== circle.length - 1) throw new Error("Fingercuffs may only join adjacent bunnies.");
  if (hasBarrierBetweenPlayers(game, circle[positions[0]].playerIndex, circle[positions[1]].playerIndex)) throw new Error("A Barrier separates those bunnies.");
  game.fingercuffs ||= [];
  game.fingercuffs.push({ id: `fingercuffs-${pending.card.id}`, card: pending.card, bunnyIds: ids });
  game.pendingAction = null;
  pushLog(game, `${circle[positions[0]].bunny.name} and ${circle[positions[1]].bunny.name} now share modifiers and fate through Fingercuffs.`);
  return finishPendingEffect(game, pending);
}

function resolveHiLowCash(game, playerIndex, card, random, options) {
  const dice = [
    { value: rollDie(12, random), sides: 12, color: "blue" },
    { value: rollDie(12, random), sides: 12, color: "red" },
    { value: rollDie(20, random), sides: 20, color: "clear" },
  ];
  const values = dice.map((die) => die.value);
  const credit = Math.max(...values) - Math.min(...values);
  const player = game.players[playerIndex];
  player.greenStoreCredit = (player.greenStoreCredit || 0) + credit;
  player.greenStoreCreditTurnNumber = game.turnNumber;
  recordDiceRoll(game, dice, card.name, player.name);
  game.discardPile.push(card);
  pushLog(game, `${player.name} received ${credit} temporary store Dolla from ${card.name}.`);
  const continued = finishCardEffect(game, options);
  continued.message = `${player.name} may spend ${credit} temporary Dolla at any open store before drawing.`;
  return continued;
}

function resolveWheelsChocolate(game, playerIndex, card, random, options) {
  const colors = ["violet", "orange", "green", "yellow", "blue", "black", "red", "pink"];
  const dice = colors.map((color) => ({ value: rollDie(12, random), sides: 12, color }));
  recordDiceRoll(game, dice, card.name);
  const high = Math.max(...dice.map((die) => die.value));
  const winners = dice.filter((die) => die.value === high).map((die) => die.color);
  const packMatches = { violet: "violet", orange: "orange", green: "green", yellow: "yellow", blue: "blue", black: "onyx", red: "red", pink: "pink" };
  const player = game.players[playerIndex];
  const taken = [];
  for (const color of winners) {
    for (let count = 0; count < 3; count += 1) {
      let index = -1;
      for (let candidateIndex = game.mainDeck.length - 1; candidateIndex >= 0; candidateIndex -= 1) {
        if (String(game.mainDeck[candidateIndex].packId || "").includes(packMatches[color])) { index = candidateIndex; break; }
      }
      if (index < 0) break;
      const [drawn] = game.mainDeck.splice(index, 1);
      taken.push(drawn);
      if (drawn.kind === "money") player.bank.push(drawn);
      else if (drawn.kind === "zodiac") player.zodiacCards.push(drawn);
      else if (isPlayImmediately(drawn)) game.immediateQueue.push({ playerIndex, card: drawn });
      else player.hand.push(drawn);
    }
  }
  game.discardPile.push(card);
  pushLog(game, `${player.name} rolled ${winners.join(", ")} highest and collected ${taken.length} matching Draw Pile card${taken.length === 1 ? "" : "s"}.`);
  if (game.immediateQueue.length) return startNextQueuedImmediate(game, { returnPhase: "draw", resumePlayerIndex: options.resumePlayerIndex ?? playerIndex });
  return finishCardEffect(game, options);
}

function resetPlayerForZodiacTerminator(game, targetIndex) {
  const target = game.players[targetIndex];
  game.carrotMarket.push(...target.carrots.splice(0));
  const discarded = [target.topRun, target.bottomRun, ...target.hand, ...target.savedSpecials, ...(target.mysteriousPlaces || []), ...target.bank, ...target.zodiacCards].filter(Boolean);
  game.discardPile.push(...discarded);
  target.topRun = null;
  target.bottomRun = null;
  target.hand = [];
  target.savedSpecials = [];
  target.mysteriousPlaces = [];
  target.bank = [];
  target.zodiacCards = [];
  target.dollaCredit = 0;
  let guard = 0;
  while (target.hand.length < 7 && game.mainDeck.length && guard < 100) {
    guard += 1;
    const drawn = drawMain(game);
    if (drawn.kind === "money") target.bank.push(drawn);
    else if (drawn.kind === "zodiac") target.zodiacCards.push(drawn);
    else if (isPlayImmediately(drawn)) {
      game.discardPile.push(drawn);
      pushLog(game, `${drawn.name} was discarded during the Zodiac Terminator restart.`);
    } else target.hand.push(drawn);
  }
}

function resolveZodiacTerminator(game, playerIndex, card, random, options) {
  const today = new Date().getDate();
  const colors = ["violet", "orange", "green", "yellow", "blue", "black", "red"];
  const zodiac = ZODIAC_SIGNS[rollDie(12, random) - 1];
  const dice = [
    ...colors.map((color) => ({ value: rollDie(12, random), sides: 12, color })),
    { value: rollDie(20, random), sides: 20, color: "clear" },
    { value: zodiac, sides: 12, color: "zodiac" },
  ];
  recordDiceRoll(game, dice, card.name);
  const targetIndex = game.players.findIndex((target) => (target.zodiacCards || []).some((entry) => entry.sign === zodiac));
  if (today <= 20 && dice.some((die) => die.color !== "zodiac" && die.value === today) && targetIndex >= 0) {
    resetPlayerForZodiacTerminator(game, targetIndex);
    const target = game.players[targetIndex];
    game.phase = "zodiacResetRun";
    game.pendingAction = { playerIndex: targetIndex, cardPlayerIndex: playerIndex, effect: "zodiacTerminator", card, ...effectReturnState(options) };
    game.message = `${target.name}: Zodiac Terminator reset your cards. Choose TOP RUN first and BOTTOM RUN second.`;
    pushLog(game, `${target.name} was reset by Zodiac Terminator after ${zodiac} and today's date (${today}) were rolled.`);
    return game;
  }
  game.discardPile.push(card);
  pushLog(game, `${card.name} rolled ${zodiac}, but did not reset a player.`);
  return finishCardEffect(game, options);
}

function resolveGiftCertificate(game, playerIndex, card, random, options) {
  if (!getKaballasMarket(game).isOpen) {
    game.discardPile.push(card);
    pushLog(game, `${card.name} could not be used because Kaballa's Market was closed.`);
    return finishCardEffect(game, options);
  }
  const credit = rollDie(12, random);
  const player = game.players[playerIndex];
  player.greenStoreCredit = (player.greenStoreCredit || 0) + credit;
  player.greenStoreCreditTurnNumber = game.turnNumber;
  player.kaballasGiftTurnNumber = game.turnNumber;
  recordDiceRoll(game, [{ value: credit, sides: 12, color: "yellow" }], card.name);
  game.discardPile.push(card);
  pushLog(game, `${player.name} received ${credit} temporary Dolla for Kaballa's Market.`);
  return finishCardEffect(game, options);
}

function resolveLeifCarrotson(game, pending, targetPlayerIndex, random) {
  const target = game.players[targetPlayerIndex];
  if (!target?.bunnies.length) throw new Error("Choose a player with at least one bunny.");
  const passengers = target.bunnies.splice(0);
  for (const bunny of passengers) removeKillerBunnyFromCircle(game, bunny.id);
  game.leifCarrotson = {
    card: pending.card,
    passengers,
    visitorIndex: (targetPlayerIndex + (game.turnDirection || 1) + game.players.length) % game.players.length,
    direction: game.turnDirection || 1,
    nextVisitTurnNumber: game.turnNumber + game.players.length,
  };
  game.pendingAction = null;
  pushLog(game, `${target.name}'s ${passengers.length} bunnies boarded ${pending.card.name} and began roaming clockwise.`);
  return finishPendingEffect(game, pending);
}

function beginLeifCarrotsonVisit(game, turnEndingPlayerIndex, summaries) {
  const leif = game.leifCarrotson;
  game.phase = "leifRoll";
  game.pendingAction = {
    playerIndex: leif.visitorIndex,
    effect: "leifCarrotsonVisit",
    card: leif.card,
    turnEndingPlayerIndex,
    turnEndSummaries: summaries,
  };
  game.message = `${game.players[leif.visitorIndex].name}: Leif Carrotson has arrived. Roll the Violet, Red, and Black d12s.`;
  return game;
}

function resolveLeifCarrotsonRoll(game, pending, random) {
  const dice = ["violet", "red", "black"].map((color) => ({ value: rollDie(12, random), sides: 12, color }));
  recordDiceRoll(game, dice, pending.card.name);
  const high = Math.max(...dice.map((die) => die.value));
  const highest = dice.filter((die) => die.value === high);
  if (highest.length !== 1) {
    game.message = `${game.players[pending.playerIndex].name}: the highest Leif Carrotson roll tied. Roll all three dice again.`;
    return game;
  }
  if (highest[0].color === "violet") {
    pushLog(game, `${game.players[pending.playerIndex].name} rolled Violet high and Leif Carrotson kept every passenger.`);
    return finishLeifCarrotsonVisit(game, pending);
  }
  game.phase = "leifPassenger";
  game.pendingAction = { ...pending, passengerMode: highest[0].color === "red" ? "take" : "discard" };
  game.message = highest[0].color === "red"
    ? `${game.players[pending.playerIndex].name}: Red was high. Choose one passenger bunny to take.`
    : `${game.players[pending.playerIndex].name}: Black was high. Choose one passenger bunny to throw overboard.`;
  return game;
}

function resolveLeifCarrotsonPassenger(game, pending, bunnyId) {
  const leif = game.leifCarrotson;
  const index = leif?.passengers.findIndex((bunny) => bunny.id === bunnyId) ?? -1;
  if (index < 0) throw new Error("Choose one bunny riding Leif Carrotson.");
  const [bunny] = leif.passengers.splice(index, 1);
  if (pending.passengerMode === "take") {
    game.players[pending.playerIndex].bunnies.push(bunny);
    addKillerBunnyToCircle(game, bunny.id);
    pushLog(game, `${game.players[pending.playerIndex].name} took ${bunny.name} from Leif Carrotson.`);
  } else {
    discardBunny(game, bunny);
    pushLog(game, `${game.players[pending.playerIndex].name} threw ${bunny.name} overboard from Leif Carrotson.`);
  }
  return finishLeifCarrotsonVisit(game, pending);
}

function finishLeifCarrotsonVisit(game, pending) {
  const leif = game.leifCarrotson;
  if (!leif.passengers.length) {
    game.discardPile.push(leif.card);
    game.leifCarrotson = null;
    pushLog(game, "Leif Carrotson was discarded after its final passenger left.");
  } else {
    let nextVisitor = (leif.visitorIndex + leif.direction + game.players.length) % game.players.length;
    if (hasBarrierBetweenPlayers(game, leif.visitorIndex, nextVisitor)) {
      leif.direction *= -1;
      nextVisitor = (leif.visitorIndex + leif.direction + game.players.length) % game.players.length;
    }
    leif.visitorIndex = nextVisitor;
    leif.nextVisitTurnNumber += game.players.length;
  }
  game.pendingAction = null;
  return completeTurnAdvance(game, pending.turnEndingPlayerIndex, pending.turnEndSummaries || []);
}

function resolveOperationSpoilsport(game, pending, bunnyIds) {
  const target = game.players[pending.targetPlayerIndex];
  const required = Math.min(pending.maxKills || 1, target?.bunnies.length || 0);
  const ids = [...new Set(bunnyIds)];
  if (ids.length !== required || ids.some((id) => !target.bunnies.some((bunny) => bunny.id === id))) {
    throw new Error(`Choose exactly ${required} of ${target?.name || "the opponent"}'s bunnies.`);
  }
  const eliminated = target.bunnies.filter((bunny) => ids.includes(bunny.id));
  target.bunnies = target.bunnies.filter((bunny) => !ids.includes(bunny.id));
  for (const bunny of eliminated) discardBunny(game, bunny, pending.playerIndex);
  game.lastBunnyDeathEvent = null;
  game.discardPile.push(pending.card);
  game.pendingAction = null;
  pushLog(game, `${game.players[pending.playerIndex].name} eliminated ${eliminated.map((bunny) => bunny.name).join(", ")} with Operation Spoilsport.`);
  if (pending.returnPhase === "draw") {
    game.phase = "draw";
    game.currentPlayerIndex = pending.resumePlayerIndex;
    game.message = "Operation Spoilsport resolved. Click the main draw pile.";
    return game;
  }
  return resumePlay(game, pending.resumePlayerIndex);
}

function resolveAlbinoAssignment(game, pending, targetPlayerIndex) {
  if (!game.players[targetPlayerIndex]) throw new Error("Choose a player for Albino Baby 5000.");
  pending.card.protectedPlayerIndex = targetPlayerIndex;
  game.players[pending.cardPlayerIndex ?? pending.playerIndex].savedSpecials.push(pending.card);
  game.pendingAction = null;
  pushLog(game, `${game.players[pending.cardPlayerIndex ?? pending.playerIndex].name} assigned Albino Baby 5000 to protect ${game.players[targetPlayerIndex].name}.`);
  return finishPendingEffect(game, pending);
}

function recordOpponentBunnyKill(game, victimPlayerIndex, killerPlayerIndex) {
  if (!Number.isInteger(killerPlayerIndex) || killerPlayerIndex === victimPlayerIndex) return;
  const previous = game.lastBunnyDeathEvent;
  game.lastBunnyDeathEvent = previous?.victimPlayerIndex === victimPlayerIndex
    && previous.killerPlayerIndex === killerPlayerIndex && previous.turnNumber === game.turnNumber
    ? { ...previous, count: previous.count + 1 }
    : { victimPlayerIndex, killerPlayerIndex, count: 1, turnNumber: game.turnNumber };
}

function resolveJuxtapose(game, playerIndex, card, random, options) {
  const firstIndex = rollDie(12, random) - 1;
  const secondIndex = (firstIndex + 6) % 12;
  const signs = [ZODIAC_SIGNS[firstIndex], ZODIAC_SIGNS[secondIndex]];
  recordDiceRoll(game, signs.map((value) => ({ value, sides: 12, color: "zodiac" })), card.name);
  const owners = signs.map((sign) => game.players.findIndex((player) => (player.zodiacCards || []).some((zodiac) => zodiac.sign === sign)));
  if (owners[0] >= 0 && owners[1] >= 0 && owners[0] !== owners[1]) {
    const first = game.players[owners[0]];
    const second = game.players[owners[1]];
    for (const field of ["carrots", "savedSpecials", "mysteriousPlaces"]) [first[field], second[field]] = [second[field] || [], first[field] || []];
    pushLog(game, `${first.name} and ${second.name} exchanged Carrots, saved Specials, and Mysterious Places.`);
  } else pushLog(game, `${card.name} rolled ${signs.join(" / ")}, but two different holders were not present.`);
  game.discardPile.push(card);
  return finishCardEffect(game, options);
}

function currentZodiacSign(date = new Date()) {
  const month = date.getMonth() + 1;
  const day = date.getDate();
  const boundaries = [[1, 20, "Aquarius"], [2, 19, "Pisces"], [3, 21, "Aries"], [4, 20, "Taurus"], [5, 21, "Gemini"], [6, 21, "Cancer"], [7, 23, "Leo"], [8, 23, "Virgo"], [9, 23, "Libra"], [10, 23, "Scorpio"], [11, 22, "Sagittarius"], [12, 22, "Capricorn"]];
  const current = boundaries.find(([boundaryMonth, boundaryDay]) => month === boundaryMonth && day >= boundaryDay);
  return current?.[2] || boundaries[(month + 10) % 12][2];
}

function resolvePrecession(game, pending, birthZodiac, random) {
  if (!ZODIAC_SIGNS.includes(birthZodiac)) throw new Error("Choose a birth Zodiac sign.");
  const player = game.players[pending.playerIndex];
  player.birthZodiac = birthZodiac;
  const attempts = Math.max(1, new Date().getMonth() + 1);
  const colors = ["violet", "orange", "green", "yellow", "blue", "black", "red"];
  let winningDice = null;
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    const zodiac = ZODIAC_SIGNS[rollDie(12, random) - 1];
    const numeric = colors.map((color) => ({ value: rollDie(12, random), sides: 12, color }));
    const values = numeric.map((die) => die.value);
    const signMatches = zodiac === birthZodiac || zodiac === currentZodiacSign() || zodiac === "Pisces";
    if (signMatches && [2, 1, 6, 10].every((value) => values.includes(value))) {
      winningDice = [...numeric, { value: zodiac, sides: 12, color: "zodiac" }];
      break;
    }
    if (attempt === attempts - 1) winningDice = [...numeric, { value: zodiac, sides: 12, color: "zodiac" }];
  }
  recordDiceRoll(game, winningDice, pending.card.name);
  const won = winningDice.some((die) => die.color === "zodiac" && [birthZodiac, currentZodiacSign(), "Pisces"].includes(die.value))
    && [2, 1, 6, 10].every((value) => winningDice.some((die) => die.color !== "zodiac" && die.value === value));
  if (won) {
    const zodiacCards = game.mainDeck.filter((candidate) => candidate.kind === "zodiac");
    game.mainDeck = game.mainDeck.filter((candidate) => candidate.kind !== "zodiac");
    player.zodiacCards.push(...zodiacCards);
    pushLog(game, `${player.name} completed Precession and claimed ${zodiacCards.length} remaining Zodiac cards.`);
  } else pushLog(game, `${player.name} did not complete Precession in ${attempts} attempt${attempts === 1 ? "" : "s"}.`);
  game.discardPile.push(pending.card);
  game.pendingAction = null;
  return finishPendingEffect(game, pending);
}

function resolveLaTapeWorm(game, pending, obligationId) {
  const obligations = game.players.flatMap((player, targetPlayerIndex) => (player.feedingObligations || []).map((obligation) => ({ obligation, targetPlayerIndex })));
  const selected = obligations.find(({ obligation }) => obligation.id === obligationId);
  if (!selected) throw new Error("Choose an active Feed the Bunny obligation.");
  selected.obligation.cabbageCost *= 2;
  game.discardPile.push(pending.card);
  game.pendingAction = null;
  pushLog(game, `${pending.card.name} doubled ${selected.obligation.card.name}'s Cabbage requirement to ${selected.obligation.cabbageCost}.`);
  return finishPendingEffect(game, pending);
}

function resolveZodiacResetRun(game, pending, cardIds) {
  const player = game.players[pending.playerIndex];
  const ids = [...new Set(cardIds)];
  if (ids.length !== 2 || ids.some((id) => !player.hand.some((card) => card.id === id))) throw new Error("Choose exactly two hand cards for TOP and BOTTOM RUN.");
  player.topRun = removeFirst(player.hand, (card) => card.id === ids[0]);
  player.bottomRun = removeFirst(player.hand, (card) => card.id === ids[1]);
  game.pendingAction = null;
  game.discardPile.push(pending.card);
  pushLog(game, `${player.name} programmed new TOP and BOTTOM RUN cards after Zodiac Terminator.`);
  return finishPendingEffect(game, pending);
}
...
function resolveActivateMinilith(game, pending, action, random) {
  ...
  const colors = ["violet", "orange", "green", "yellow", "blue", "black", "red"];
  const rolls = colors.map(() => rollD12(random));
  const evenCount = rolls.filter((value) => value % 2 === 0).length;
  recordDiceRoll(game, rolls.map((value, index) => ({ value, sides: 12, color: colors[index] })), pending.card.name, holder.name);
  if (evenCount === 7) {
...
function resolveBarrierPlacement(game, pending, leftPlayerIndex) {
  ...
  const broken = (game.fingercuffs || []).filter((cuffs) => {
    const entries = cuffs.bunnyIds.map((id) => findKillerBunnyInCircle(game, id));
    return entries.every(Boolean) && hasBarrierBetweenPlayers(game, entries[0].playerIndex, entries[1].playerIndex);
  });
  if (broken.length) {
    game.fingercuffs = game.fingercuffs.filter((cuffs) => !broken.includes(cuffs));
    game.discardPile.push(...broken.map((cuffs) => cuffs.card));
    pushLog(game, `${broken.length} Fingercuffs connection${broken.length === 1 ? " was" : "s were"} broken by the new Barrier.`);
  }
  game.pendingAction = null;
...
function resolveHempRoll(game, pending, random) {
  const value = rollD12(random);
  recordDiceRoll(game, [{ value, sides: 12, color: "yellow" }], pending.card.name);
  const abducted = game.area51Abducted;
...
function resolveRainbo(game, pending, targetPlayerIndex, random = Math.random) {
  ...
  recordDiceRoll(game, rolls.map((roll) => ({ ...roll, sides: 12 })), pending.card.name);
  game.discardPile.push(pending.card);
  game.pendingAction = null;
...
function canAntiMissileCancel(game, playerIndex) {
  const pending = game.pendingAction;
  return [48, 181, 200, 292, 294].includes(Number(pending?.card?.number))
    && ["target", "defend", "areaWeaponRoll", "roamingTarget", "roamingRoll"].includes(game.phase)
    && (pending.targetPlayerIndex === playerIndex || game.phase === "target" || game.phase === "roamingTarget");
}

function cancelWithAntiMissile(game, playerIndex, card) {
  if (!canAntiMissileCancel(game, playerIndex)) throw new Error("Anti-Missile cannot stop the current card.");
  const pending = game.pendingAction;
  game.discardPile.push(card);
  discardUsedWeapon(game, pending.card);
  game.pendingAction = null;
  pushLog(game, `${game.players[playerIndex].name} used ${card.name} to eliminate ${pending.card.name}.`);
  return continueAfterWeapon(game, pending);
}

function cancelVerySpecialWithUriel(game, playerIndex, card) {
  const pending = game.pendingAction;
  if (pending?.card?.type !== "VERY SPECIAL" || pending.playerIndex === playerIndex) throw new Error("Uriel's Machine needs an opponent's Very Special awaiting resolution.");
  game.discardPile.push(card, pending.card);
  game.pendingAction = null;
  pushLog(game, `${game.players[playerIndex].name} canceled ${pending.card.name} with ${card.name}.`);
  return finishPendingEffect(game, pending);
}

function cancelFreshnessWithWeilerware(game, playerIndex, card) {
  const pending = game.pendingAction;
  if (game.phase !== "freshnessChoice" || pending?.targetPlayerIndex !== playerIndex) throw new Error("Weilerware needs Weil's Freshness Center targeting you.");
  game.discardPile.push(card, pending.card);
  game.pendingAction = null;
  pushLog(game, `${game.players[playerIndex].name} canceled ${pending.card.name} with ${card.name}.`);
  return finishPendingEffect(game, pending);
...
function beginDudeChallenge(game, pending, action, random) {
  ...
  pending.roll = value;
  recordDiceRoll(game, [{ value, sides, color: sides === 20 ? "clear" : "red" }], pending.card.name);
...
function continueDudeChallenge(game, pending, random) {
  ...
  if (pending.remainingAttempts > 0 && game.players[pending.rollerIndex].carrots.length) {
    ...
    recordDiceRoll(game, [{ value: pending.roll, sides: pending.dieSides, color: "clear" }], pending.card.name);
    game.phase = "dudeGuess";
...
function rollMysteryUrn(game, pending, random) {
  ...
  const value = rollD12(random);
  recordDiceRoll(game, [{ value, sides: 12, color: "violet" }], pending.card.name);
...
function rollMysteryUrnFinal(game, pending, random) {
  const value = rollD12(random);
  recordDiceRoll(game, [{ value, sides: 12, color: "violet" }], `${pending.card.name} final chance`);
...
function resolveSinisterBounceTarget(game, pending, bunnyId) {
  ...
  const reduction = getEffectiveCloverReduction(game, bunny);
  game.phase = "defend";
  game.pendingAction = {
    ...pending,
...
function restoreArea51Bunny(game) {
  const abducted = game.area51Abducted;
  const bunnies = abducted?.bunnies || (abducted?.bunny ? [abducted.bunny] : []);
  if (!bunnies.length || !game.players[abducted.ownerIndex]) return;
  game.players[abducted.ownerIndex].bunnies.push(...bunnies);
  for (const bunny of bunnies) addKillerBunnyToCircle(game, bunny.id);
  pushLog(game, `${game.players[abducted.ownerIndex].name}'s ${bunnies.map((bunny) => bunny.name).join(" and ")} returned from Area 51.`);
  game.area51Abducted = null;
}
...
export function chooseKillerBunniesBlueRollTarget(game, playerIndex, targetPlayerIndex, bunnyId) {
  ...
  next.pendingAction = {
    ...
    targetPlayerIndex,
    bunnyId,
    diceCount: [66, 67].includes(Number(pending.card.number)) ? 5 : [304, 305].includes(Number(pending.card.number)) ? 6 : 1,
  };
  next.message = `${target.name}: roll ${next.pendingAction.diceCount === 1 ? "the d12" : `${next.pendingAction.diceCount} colored d12s`} for ${bunny.name}.`;
  return next;
}
...
export function chooseKillerBunniesNumber(game, playerIndex, value) {
  ...
  const isIncredible = next.pendingAction.effect === "incredibleMagicFountain";
  next.pendingAction = { ...next.pendingAction, effect: isIncredible ? "incredibleMagicFountain" : "magicFountain", chosenNumber: number, diceCount: isIncredible ? 8 : 5 };
  next.message = `${next.players[playerIndex].name}: roll ${isIncredible ? "seven colored d12s and the Clear d20" : "five d12s"}. Each ${number} revives one bunny.`;
  return next;
}
...
function resolveKillerBunniesBlueSpecialRollCore(game, playerIndex, random = Math.random) {
  ...
  const next = clone(game);
  const pending = next.pendingAction;
  const superRoll = ["incredibleMagicFountain", "superSuppliesSurprise"].includes(pending.effect);
  const colors = superRoll ? ["violet", "orange", "green", "yellow", "blue", "black", "red", "clear"] : ["violet", "orange", "green", "yellow", "blue"];
  const dice = colors.map((color, index) => ({ color, sides: superRoll && index === 7 ? 20 : 12, value: rollDie(superRoll && index === 7 ? 20 : 12, random) }));
  const rolls = dice.map((die) => die.value);
  recordDiceRoll(next, dice, pending.card.name);
  if (["magicFountain", "incredibleMagicFountain"].includes(pending.effect)) {
  ...
  if (pending.effect === "superSuppliesSurprise") {
    const lowest = Math.min(...rolls);
    let winners = rolls.map((roll, index) => roll === lowest ? index : -1).filter((index) => index >= 0);
    if (winners.includes(7)) winners = [0, 1, 2, 3, 4, 5, 6];
    const player = next.players[playerIndex];
    pending.reviveCount = winners.includes(0) ? 2 : 0;
    pending.carrotCount = winners.includes(1) ? Math.min(2, next.carrotMarket.length) : 0;
    for (let count = 0; count < (winners.includes(2) ? 2 : 0) && next.cabbageSupply.length; count += 1) player.cabbage.push(next.cabbageSupply.pop());
    if (winners.includes(3)) awardDollaFromDiscard(next, player, 20);
    for (let count = 0; count < (winners.includes(4) ? 2 : 0) && next.waterSupply.length; count += 1) player.water.push(next.waterSupply.pop());
    for (let count = 0; count < (winners.includes(5) ? 2 : 0) && next.rooneysEmporium?.defenseSupply.length; count += 1) player.defenseCards.push(next.rooneysEmporium.defenseSupply.pop());
    for (let count = 0; count < (winners.includes(6) ? 2 : 0) && next.weilsPawnShop?.pawnSupply.length; count += 1) player.pawns.push(next.weilsPawnShop.pawnSupply.pop());
    pushLog(next, `${pending.card.name} triggered ${winners.length} Super Supplies reward categories.`);
    return continueBlueRewards(next, pending);
  }
...
export function chooseKillerBunniesRoamingTarget(game, playerIndex, targetPlayerIndex, bunnyId) {
  ...
  next.pendingAction = {
    ...
    bunnyId,
    playerIndex: targetPlayerIndex,
    direction: [47, 292].includes(Number(pending.card.number)) ? -1 : 1,
    power: Number(pending.card.number) === 47 ? 11 : Number(pending.card.number) === 292 ? 18 : 9,
    minilithActive: hasActiveMinilith(next, playerIndex, pending.card),
    visitedBunnyIds: [],
    initialAttack: true,
  };
  next.message = `${next.players[targetPlayerIndex].name}: roll a d${Number(pending.card.number) === 292 ? 20 : 12} for ${entry.bunny.name} against ${pending.card.name}.`;
  return next;
}
...
function resolveKillerBunniesRoamingRollCore(game, playerIndex, random = Math.random) {
  ...
  const sides = Number(pending.card.number) === 292 ? 20 : 12;
  const value = rollDie(sides, random);
  recordDiceRoll(next, [{ value, sides, color: Number(pending.card.number) === 181 ? "red" : sides === 20 ? "clear" : null }], pending.card.name);
...
function resolveKillerBunniesBlueCardRollCore(game, playerIndex, random = Math.random) {
  ...
  pending.rolls = rolls;
  const colors = pending.diceCount === 6 ? ["violet", "orange", "green", "yellow", "blue", "black"]
    : pending.diceCount === 5 ? ["violet", "orange", "green", "yellow", "blue"]
      : [[310, 311].includes(Number(pending.card.number)) ? "red" : null].flat();
  recordDiceRoll(next, rolls.map((value, index) => ({ value, sides: 12, color: colors[index] })), pending.card.name);
  const number = Number(pending.card.number);
  ...
  if ([304, 305].includes(number)) return resolveCarrotTopRoyale(next, pending, rolls);
  if ([310, 311].includes(number)) return resolveMonumentalBakery(next, pending, rolls[0]);
  throw new Error("That Blue Bunny Bits roll is not automated yet.");
}

function resolveCarrotTopRoyale(game, pending, rolls) {
  const duplicate = new Set(rolls).size !== rolls.length;
  if (duplicate) {
    eliminateCircleEntry(game, findKillerBunnyInCircle(game, pending.bunnyId), `${pending.card.name}'s duplicate roll`);
    return finishRolledBlueCard(game, pending);
  }
  const actor = game.players[pending.cardPlayerIndex];
  awardDollaFromDiscard(game, actor, 10);
  pending.playerIndex = pending.cardPlayerIndex;
  pending.carrotCount = 1;
  pending.reviveCount = 0;
  game.phase = "royaleGamble";
  game.message = `${actor.name}: all six dice were different. Keep the Carrot and 10 Dolla, or gamble with the Red d12.`;
  return game;
}

function resolveCarrotTopRoyaleGamble(game, pending, choice, random) {
  if (choice === "keep") return continueBlueRewards(game, pending);
  if (choice !== "gamble") throw new Error("Keep the reward or gamble with the Red d12.");
  const value = rollDie(12, random);
  recordDiceRoll(game, [{ value, sides: 12, color: "red" }], `${pending.card.name} gamble`);
  if (pending.rolls.includes(value)) {
    const actor = game.players[pending.cardPlayerIndex];
    for (const bunny of actor.bunnies.splice(0)) discardBunny(game, bunny);
    game.carrotMarket.push(...actor.carrots.splice(0));
    for (const money of actor.bank.splice(0)) game.discardPile.push(money);
    actor.dollaCredit = 0;
    pending.carrotCount = 0;
    pushLog(game, `${actor.name}'s Red ${value} matched and lost every bunny, Carrot, and Dolla.`);
    return finishRolledBlueCard(game, pending);
  }
  awardDollaFromDiscard(game, game.players[pending.cardPlayerIndex], 10);
  pending.reviveCount = 2;
  pushLog(game, `${pending.card.name}'s Red ${value} did not match: +10 Dolla and up to two revived bunnies.`);
  return continueBlueRewards(game, pending);
}

function resolveMonumentalBakery(game, pending, roll) {
  const target = findKillerBunnyInCircle(game, pending.bunnyId);
  const actor = game.players[pending.cardPlayerIndex];
  if (!target) return finishRolledBlueCard(game, pending);
  if (roll >= 10) {
    pending.carrotCount = roll - 9;
    pending.reviveCount = 0;
    return continueBlueRewards(game, pending);
  }
  if (roll >= 7) {
    for (let count = 0; count < 3 && game.cabbageSupply.length; count += 1) actor.cabbage.push(game.cabbageSupply.pop());
    for (let count = 0; count < 3 && game.waterSupply.length; count += 1) actor.water.push(game.waterSupply.pop());
  } else if (roll >= 4) immediatelyFeedOrEliminate(game, target, 5, 5, pending.card.name);
  else if (roll === 3) eliminateCircleEntry(game, target, pending.card.name);
  else if (roll === 2) {
    for (const entry of getKillerBunniesCircleRange(game, pending.bunnyId, 1)) eliminateCircleEntry(game, findKillerBunnyInCircle(game, entry.bunny.id), pending.card.name);
  } else {
    for (const bunny of target.player.bunnies.slice()) eliminateCircleEntry(game, findKillerBunnyInCircle(game, bunny.id), pending.card.name);
    game.carrotMarket.push(...target.player.carrots.splice(0));
  }
  return finishRolledBlueCard(game, pending);
}
...
export function chooseKillerBunniesModifierTarget(game, playerIndex, targetPlayerIndex, bunnyId) {
  ...
  if (Number(pending.card.number) === 239) {
    bunny.colors = ["red"];
    bunny.color = "red";
  }
...
function resolveKillerBunniesBlackCatRollCore(game, playerIndex, random = Math.random) {
  ...
  recordDiceRoll(next, [{ value: roll, sides: 12, color: "green" }], pending.card.name);
...
function resolveKillerBunniesPovertyPokerRollCore(game, playerIndex, choice, random = Math.random) {
  ...
  if (next.phase === "povertyPokerReroll") {
    ...
    pending.optionalRerollResolved = true;
    recordDiceRoll(next, [{ value, sides: 12, color: null }], `${pending.card.name} optional reroll`);
  ...
  recordDiceRoll(next, [{ value, sides: 12, color: null }], pending.card.name);
  pending.rollQueue.shift();
...
function resolveKillerBunniesDefenseCore(game, playerIndex, choice, random = Math.random) {
  ...
  if (pending.effect === "weapon") {
    ...
    if (choice === "defense") {
      ...
      const printedPower = pending.card.defensePower || pending.card.power;
      ...
      const seventhWhisk = Number(pending.card.number) === 347;
      const sides = Number(pending.card.power) >= 13 ? 20 : 12;
      const upgraded = Boolean(target.nextD12UsesD20) && (seventhWhisk || sides === 12);
      if (upgraded) target.nextD12UsesD20 = false;
      const dice = seventhWhisk
        ? pending.card.diceColors.map((color, index) => ({ value: rollDie(upgraded && index === 0 ? 20 : 12, random), sides: upgraded && index === 0 ? 20 : 12, color: upgraded && index === 0 ? "clear" : color }))
        : [{ value: rollDie(upgraded ? 20 : sides, random), sides: upgraded ? 20 : sides, color: upgraded || sides === 20 ? "clear" : null }];
      const roll = dice[0].value;
      recordDiceRoll(next, dice, pending.card.name);
      const weaponHits = seventhWhisk ? dice.some((die) => die.value <= effectivePower) : roll <= effectivePower;
      if (target.shields > 0) {
        target.shields -= 1;
        pushLog(next, `${target.name} rolled ${formatDiceResults(dice)}; their burrow shield stopped ${pending.card.name}.`);
        weaponFailed = true;
      } else if (weaponHits) {
        if (Number(pending.card.number) === 291) {
          bunny.comatose = true;
          pushLog(next, `${target.name} rolled ${roll}; ${bunny.name} entered a C.O.M.A. and cannot support aggressive cards.`);
        } else {
          recordOpponentBunnyKill(next, pending.targetPlayerIndex, pending.attackingPlayerIndex);
          target.bunnies.splice(bunnyIndex, 1);
          discardBunny(next, bunny, pending.attackingPlayerIndex);
          pushLog(next, `${target.name} rolled ${formatDiceResults(dice)}; ${pending.card.name} knocked out ${bunny.name}.`);
        }
      } else {
        pushLog(next, `${target.name} rolled ${formatDiceResults(dice)}; ${bunny.name} escaped ${pending.card.name}.`);
        weaponFailed = true;
      }
    } else throw new Error("Roll the d12 or use enough Defense Cards to resolve this attack.");
    if (weaponFailed && Number(pending.card.number) === 343) {
      const nextEntry = getNextKillerBunnyInCircle(next, bunny.id, 1, (entry) => !hasHeavenlyHalo(entry.bunny));
      if (nextEntry) {
        const nextPower = (pending.snowballPower || pending.card.power) + 1;
        const reduction = getEffectiveCloverReduction(next, nextEntry.bunny);
        next.phase = "defend";
        next.pendingAction = {
          ...pending,
          playerIndex: nextEntry.playerIndex,
          targetPlayerIndex: nextEntry.playerIndex,
          bunnyId: nextEntry.bunny.id,
          snowballPower: nextPower,
          cloverReduction: reduction,
          effectivePower: Math.max(0, nextPower - reduction),
        };
        next.message = `${pending.card.name} rolls clockwise to ${nextEntry.player.name}'s ${nextEntry.bunny.name} at level ${nextPower}. Roll the Clear d20.`;
        return next;
      }
    }
    if (weaponFailed && Number(pending.card.number) === 287) {
      pending.boomerangSides ||= [pending.attackingPlayerIndex, pending.targetPlayerIndex];
      const nextTargetPlayerIndex = pending.targetPlayerIndex === pending.boomerangSides[1]
        ? pending.boomerangSides[0] : pending.boomerangSides[1];
      const nextTarget = next.players[nextTargetPlayerIndex];
      const nextBunny = nextTarget?.bunnies.find((entry) => !hasHeavenlyHalo(entry) && !isIntangibleHologram(entry));
      if (nextBunny) {
        const reduction = getEffectiveCloverReduction(next, nextBunny);
        next.phase = "defend";
        next.pendingAction = {
          ...pending,
          playerIndex: nextTargetPlayerIndex,
          targetPlayerIndex: nextTargetPlayerIndex,
          bunnyId: nextBunny.id,
          cloverReduction: reduction,
          effectivePower: Math.max(0, pending.card.power - reduction),
        };
        next.message = `${pending.card.name} boomeranged to ${nextTarget.name}'s ${nextBunny.name}. Roll the Clear d20.`;
        pushLog(next, `${pending.card.name} bounced back toward ${nextTarget.name}.`);
        return next;
      }
    }
...
export function chooseKillerBunniesCarrot(game, playerIndex, carrotId) {
  ...
  if (pending?.communityCarrotQueue) {
    const queue = pending.communityCarrotQueue.filter((index) => index !== playerIndex);
    if (queue.length && next.carrotMarket.length) {
      next.pendingAction = { ...pending, playerIndex: queue[0], communityCarrotQueue: queue };
      next.message = `${next.players[queue[0]].name}: choose your Community Carrot.`;
      return next;
    }
    next.discardPile.push(pending.card);
    next.pendingAction = null;
    return next.carrotMarket.length ? finishImmediateEffect(next, pending) : beginMagicReveal(next);
  }
...
export function drawKillerBunniesPile(game, playerIndex, pile, options = {}, random = Math.random) {
  ...
  if (pile === "main") {
    ...
    if (card.kind === "money") {
      if (card.randomValue) {
        const value = rollDie(20, random);
        player.dollaCredit = (player.dollaCredit || 0) + value;
        next.discardPile.push(card);
        recordDiceRoll(next, [{ value, sides: 20, color: "clear" }], card.name, player.name);
        pushLog(next, `${player.name} rolled ${value} temporary Dolla from ${card.name}.`);
      } else {
    ...
    } else if (card.kind === "zodiac") {
      player.zodiacCards ||= [];
      player.zodiacCards.push(card);
      pushLog(next, `${player.name} saved ${card.sign || card.name} face-up and may draw again.`);
      next.message = `${card.sign || card.name} banked automatically. Click the main deck again.`;
  ...
  const couponShoppingAfterRun = next.phase === "draw" && canShopAfterRun(next, playerIndex, "kaballas");
...
function finishRunReplacement(game, playerIndex) {
  const player = game.players[playerIndex];
  if (game.restartingTurnPlayerIndex === playerIndex) {
    game.restartingTurnPlayerIndex = null;
    game.phase = "play";
    game.message = `${player.name}'s restarted turn is ready: shop or flip TOP RUN.`;
    return game;
  }
...
function finishTurn(game, playerIndex) {
  ...
  if (next.leifCarrotson?.passengers.length && next.turnNumber >= next.leifCarrotson.nextVisitTurnNumber) {
    return beginLeifCarrotsonVisit(next, playerIndex, summaries);
  }
...
function completeTurnAdvance(next, playerIndex, summaries = []) {
  next.players[playerIndex].greenStoreCredit = 0;
  next.players[playerIndex].kaballasGiftTurnNumber = null;
  enforcePersistentBunnyRequirements(next);
  const direction = next.turnDirection || 1;
  next.currentPlayerIndex = (playerIndex + direction + next.players.length) % next.players.length;
  next.turnNumber += 1;
  let skipped = 0;
  while ((next.players[next.currentPlayerIndex].skipTurns || 0) > 0 && skipped < next.players.length) {
    ...
    next.currentPlayerIndex = (next.currentPlayerIndex + direction + next.players.length) % next.players.length;
    next.turnNumber += 1;
    skipped += 1;
  }
...
export function runKillerBunniesComputers(game, random = Math.random) {
  ...
  while (steps < 10000 && next.phase !== "gameOver" && next.phase !== "reveal") {
    ...
    if (next.phase === "setupRun") {
      ...
      const canSaveMinilith = specialNumber !== 147 || player.bunnies.some(isLivingBunny);
      next = resolveKillerBunniesSpecialChoice(next, playerIndex, [80, 81, 145, 147, 148, 152, 203, 204].includes(specialNumber) && canSaveMinilith ? "save" : "use", random);
      ...
      const target = next.pendingAction.allowOwnTarget
        ...
        : findBotTarget(next, playerIndex, next.pendingAction.card);
      if (!target) {
...
export function getKillerBunniesPileStatus(game, playerIndex, pile) {
  ...
  const couponShoppingAfterRun = isCurrent && game.phase === "draw" && canShopAfterRun(game, playerIndex, "kaballas");
...
export function getKillerBunniesCardPlayStatus(player, card) {
  ...
  const eligibleBunnyCount = (player?.bunnies || []).filter((bunny) => !hasHeavenlyHalo(bunny) && isLivingBunny(bunny) && !bunny.comatose).length;
...
export function getKillerBunniesShopItemStatus(game, playerIndex, shop, item, cardId) {
  ...
  const couponShoppingAfterRun = game.phase === "draw" && canShopAfterRun(game, playerIndex, shop);
  ...
  if (shop === "rooneys") {
    ...
    const basePrice = item === "weapon"
      ? store.weaponPriceMode === "half-level" ? Math.ceil((card?.power || 0) / 2) : card?.power || 0
      : store.defensePrice;
  ...
  if (shop === "weils") {
    ...
    const basePrice = item === "pawn" ? store.pawnPrice : store.bunnyPrice;
    const price = isWeilsCouponActive(game, playerIndex) ? Math.ceil(basePrice / 2) : basePrice;
...
function resolveRunCard(game, playerIndex, card, random, options = {}) {
  ...
  if ([175, 230].includes(Number(card.number))) {
    ...
    game.pendingAction = {
      ...
      card: {
        ...card,
        cabbageCost: Number(card.number) === 230 ? 3 : 1,
        waterCost: Number(card.number) === 230 ? 3 : 1,
      },
      ...effectReturnState(options),
    };
    game.message = `Choose an opponent. Every bunny they currently own must feed ${Number(card.number) === 230 ? "3 Cabbage and 3 Water" : "1 Cabbage and 1 Water"}.`;
    return game;
  }
  if ([185, 186, 238].includes(Number(card.number))) {
  ...
  if (Number(card.number) === 242) {
    player.insightCarrots = game.magicCarrotDeck.slice(-5).map((entry) => ({ id: entry.id, name: entry.name, carrotKey: entry.carrotKey }));
    game.discardPile.push(card);
    pushLog(game, `${player.name} privately viewed the top ${player.insightCarrots.length} Magic Carrot cards with Insight.`);
    const continued = finishCardEffect(game, options);
    continued.message = `Insight revealed ${player.insightCarrots.length} possible Magic Carrots on your player mat.${continued.phase === "draw" ? " Click the main draw pile." : ""}`;
    return continued;
  }
  if (Number(card.number) === 243) {
    game.phase = "lowJackRoll";
    game.pendingAction = { playerIndex, effect: "lowJackKojak", card, ...effectReturnState(options) };
    game.message = `${player.name}: roll the Clear d20 for Low Jack Kojak.`;
    return game;
  }
  if (Number(card.number) === 247) return resolveShareTheWealth(game, playerIndex, card, options);
  if (Number(card.number) === 248) {
    const colorCounts = {};
    for (const bunny of player.bunnies) for (const color of bunnyColors(bunny)) colorCounts[color] = (colorCounts[color] || 0) + 1;
    const reviveCount = Object.values(colorCounts).reduce((total, count) => total + Math.floor(count / 2), 0);
    const pending = { playerIndex, cardPlayerIndex: playerIndex, effect: "blownTrojan", card, reviveCount, carrotCount: 0, ...effectReturnState(options) };
    if (reviveCount > 0) return continueBlueRewards(game, pending);
    const trojan = removeFirst(game.rooneysEmporium?.weaponDiscard || [], (entry) => Number(entry.number) === 46)
      || removeFirst(game.discardPile, (entry) => Number(entry.number) === 46);
    if (trojan) game.discardPile.push(trojan);
    game.discardPile.push(card);
    return finishCardEffect(game, options);
  }
  if (Number(card.number) === 249) {
    game.phase = "bunnyHop";
    game.pendingAction = { playerIndex, effect: "bunnyHop", card, ...effectReturnState(options) };
    game.message = "Choose a boundary. The two adjacent players exchange both RUN cards.";
    return game;
  }
  if (Number(card.number) === 250) {
    game.phase = "coolChange";
    game.pendingAction = { playerIndex, effect: "coolChange", card, ...effectReturnState(options) };
    game.message = "Choose any cards from your hand and RUN slots to replace from the Draw Pile.";
    return game;
  }
  if (Number(card.number) === 252) {
    const obligations = game.players.flatMap((target, targetPlayerIndex) => (target.feedingObligations || []).map((obligation) => ({ ...obligation, targetPlayerIndex })));
    if (!obligations.length) {
      game.discardPile.push(card);
      return finishCardEffect(game, options);
    }
    game.phase = "laHotPeppers";
    game.pendingAction = { playerIndex, effect: "laHotPeppers", card, obligations, ...effectReturnState(options) };
    game.message = "Choose one pending Feed the Bunny card. Its Water requirement will double.";
    return game;
  }
  if (Number(card.number) === 254) {
    game.phase = "paintballTarget";
    game.pendingAction = { playerIndex, effect: "randomPaintball", card, ...effectReturnState(options) };
    game.message = "Choose a bunny or Pawn for Random Paintball Assault.";
    return game;
  }
  if (Number(card.number) === 255) {
    game.phase = "runAmokTarget";
    game.pendingAction = { playerIndex, effect: "runAmok", card, ...effectReturnState(options) };
    game.message = "Choose an opponent. Their TOP and BOTTOM RUN cards will be discarded and replaced.";
    return game;
  }
  if (Number(card.number) === 256) {
    const eligibleIds = player.hand.filter((entry) => entry.type === "RUN" && entry.id !== card.id).map((entry) => entry.id);
    if (!eligibleIds.length) {
      game.discardPile.push(card);
      return finishCardEffect(game, options);
    }
    game.phase = "runTransformer";
    game.pendingAction = { playerIndex, effect: "runTransformer", card, eligibleIds, ...effectReturnState(options) };
    game.message = "Choose one RUN card from your hand to transform and play immediately.";
    return game;
  }
  if (Number(card.number) === 259) return resolveBunniesInBlack(game, playerIndex, card, options);
  if (Number(card.number) === 260) {
    game.turnDirection = (game.turnDirection || 1) * -1;
    game.discardPile.push(card);
    pushLog(game, `${player.name} reversed the direction of play.`);
    return finishCardEffect(game, options);
  }
  if ([251, 258, 317, 318].includes(Number(card.number))) {
    player.savedSpecials ||= [];
    player.savedSpecials.push(card);
    pushLog(game, `${player.name} kept ${card.name} ready for its matching threat.`);
    return finishCardEffect(game, options);
  }
  if (Number(card.number) === 316) {
    game.runPlaysThisTurn = 0;
    game.purchases = { cabbage: false, water: false, carrot: false };
    game.restartingTurnPlayerIndex = playerIndex;
    game.phase = "draw";
    game.pendingAction = null;
    game.discardPile.push(card);
    game.message = `${player.name} restarted their turn with Bunny On The Edge Of Forever. Draw and refill BOTTOM RUN first.`;
    pushLog(game, `${player.name} restarted the playable portion of their turn.`);
    return game;
  }
  if (Number(card.number) === 348) {
    game.phase = "dayDeadRoll";
    game.pendingAction = { playerIndex, cardPlayerIndex: playerIndex, effect: "dayOfTheDead", card, ...effectReturnState(options) };
    game.message = `${player.name}: roll each of the seven colored d12s three times for Day Of The Dead.`;
    return game;
  }
  if (Number(card.number) === 349) {
    if (getKillerBunniesCircleEntries(game).length < 2) {
      game.discardPile.push(card);
      return finishCardEffect(game, options);
    }
    game.phase = "fingercuffs";
    game.pendingAction = { playerIndex, effect: "fingercuffs", card, ...effectReturnState(options) };
    game.message = "Choose two adjacent bunnies for Fingercuffs.";
    return game;
  }
  if (Number(card.number) === 350) return resolveHiLowCash(game, playerIndex, card, random, options);
  if (Number(card.number) === 351) return resolveJuxtapose(game, playerIndex, card, random, options);
  if (Number(card.number) === 352) {
    const targets = game.players.map((target, index) => target.bunnies.length ? index : null).filter(Number.isInteger);
    if (!targets.length) {
      game.discardPile.push(card);
      return finishCardEffect(game, options);
    }
    game.phase = "leifTarget";
    game.pendingAction = { playerIndex, effect: "leifCarrotson", card, targetIndexes: targets, ...effectReturnState(options) };
    game.message = "Choose the player whose bunnies will board Leif Carrotson.";
    return game;
  }
  if (Number(card.number) === 354) {
    game.phase = "precessionBirth";
    game.pendingAction = { playerIndex, effect: "precession", card, ...effectReturnState(options) };
    game.message = "Choose your birth Zodiac sign, then make this month's Precession attempts.";
    return game;
  }
  if (Number(card.number) === 357) return resolveWheelsChocolate(game, playerIndex, card, random, options);
  if (Number(card.number) === 358) return resolveZodiacTerminator(game, playerIndex, card, random, options);
  if (Number(card.number) === 360) return resolveGiftCertificate(game, playerIndex, card, random, options);
  if (Number(card.number) === 361) {
    const obligations = game.players.flatMap((target, targetPlayerIndex) => (target.feedingObligations || []).map((obligation) => ({ ...obligation, targetPlayerIndex })));
    if (!obligations.length) {
      game.discardPile.push(card);
      return finishCardEffect(game, options);
    }
    game.phase = "laTapeWorm";
    game.pendingAction = { playerIndex, effect: "laTapeWorm", card, obligations, ...effectReturnState(options) };
    game.message = "Choose one pending Feed the Bunny card. Its Cabbage requirement will double.";
    return game;
  }
  if (Number(card.number) === 362) {
    game.phase = "albinoTarget";
    game.pendingAction = { playerIndex, cardPlayerIndex: playerIndex, effect: "albinoBaby", card, ...effectReturnState(options) };
    game.message = "Choose the player whose bunnies Albino Baby 5000 will protect.";
    return game;
  }
  if (Number(card.number) === 363) {
    game.discardPile.push(card);
    pushLog(game, `${card.name} had no recorded opponent-caused bunny loss to mirror.`);
    return finishCardEffect(game, options);
  }
  if (Number(card.number) === 364) {
    player.nextD12UsesD20 = true;
    game.discardPile.push(card);
    pushLog(game, `${player.name}'s next numeric d12 roll will use the Clear d20.`);
    return finishCardEffect(game, options);
  }
  ...
  if ([49, 50, 72, 73, 134, 253].includes(Number(card.number))) {
  ...
  if (Number(card.number) === 295 || card.kind === "allStoresOpen") {
    game.kaballasMarket = structuredClone(KABALLAS_MARKET_STARTER);
    if (game.rooneysEmporium) {
      const inventory = { defenseSupply: game.rooneysEmporium.defenseSupply, weaponDiscard: game.rooneysEmporium.weaponDiscard };
      game.rooneysEmporium = { ...structuredClone(ROONEYS_WEAPONS_EMPORIUM_STARTER), ...inventory };
    }
    if (game.weilsPawnShop) {
      const inventory = { pawnSupply: game.weilsPawnShop.pawnSupply, bunnyDiscard: game.weilsPawnShop.bunnyDiscard };
      game.weilsPawnShop = { ...structuredClone(WEILS_PAWN_SHOP_STARTER), ...inventory };
    }
    game.discardPile.push(card);
    pushLog(game, `${player.name} restored every available store to its starter prices.`);
    return finishCardEffect(game, options);
  }
  if (Number(card.number) === 299) {
    game.phase = "playerTarget";
    game.pendingAction = { playerIndex, effect: "playerTarget", card, ...effectReturnState(options) };
    game.message = "Choose a player whose vulnerable bunnies will be abducted by Mothership.";
    return game;
  }
  if (Number(card.number) === 300) {
    game.phase = "pilferPawnRoll";
    game.pendingAction = { playerIndex, effect: "pilferPawn", card, ...effectReturnState(options) };
    game.message = `${player.name}: roll the colored d12s to pilfer every Pawn tied for the highest result.`;
    return game;
  }
  if (Number(card.number) === 302) {
    game.selectiveDisclosureBy ||= [];
    if (!game.selectiveDisclosureBy.includes(playerIndex)) game.selectiveDisclosureBy.push(playerIndex);
    game.discardPile.push(card);
    pushLog(game, `${player.name} exposed every opponent's programmed RUN cards with Selective Disclosure.`);
    return finishCardEffect(game, options);
  }
  if ([304, 305, 310, 311].includes(Number(card.number))) {
    if (!game.players.some((target) => target.bunnies.length)) {
      game.discardPile.push(card);
      return finishCardEffect(game, options);
    }
  ...
  if (Number(card.number) === 306) {
    if (!game.weilsPawnShop?.isOpen || !game.weilsPawnShop.pawnSupply.length) {
      game.discardPile.push(card);
      return finishCardEffect(game, options);
    }
    game.phase = "freePawnChoice";
    game.pendingAction = { playerIndex, effect: "freePawn", card, ...effectReturnState(options) };
    game.message = `Choose up to ${Math.min(2, game.weilsPawnShop.pawnSupply.length)} Pawns from Weil's Pawn Shop.`;
    return game;
  }
  if (Number(card.number) === 307) {
    player.weilsCouponTurnNumber = game.turnNumber;
    game.discardPile.push(card);
    pushLog(game, `${player.name} activated half-price shopping at Weil's Pawn Shop for this turn.`);
    const continued = finishCardEffect(game, options);
    continued.message = `Weil's Pawn Shop is half price for ${player.name} through the end of this turn.${continued.phase === "draw" ? " You may shop before drawing." : ""}`;
    return continued;
  }
  if ([308, 309].includes(Number(card.number))) {
    game.phase = "numberChoice";
    game.pendingAction = { playerIndex, effect: "incredibleMagicFountain", card, ...effectReturnState(options) };
    game.message = "Choose a lucky number from 1 through 12 for the Incredibly Magic Fountain.";
    return game;
  }
  if ([312, 313].includes(Number(card.number))) {
    game.phase = "blueSpecialRoll";
    game.pendingAction = { playerIndex, effect: "superSuppliesSurprise", card, ...effectReturnState(options) };
    game.message = `${player.name}: roll seven colored d12s and the Clear d20 for Super Supplies Surprise.`;
    return game;
  }
  if ([314, 315].includes(Number(card.number))) {
    let cleared = 0;
    for (const target of game.players) {
      const obligations = target.feedingObligations || [];
      cleared += obligations.length;
      target.feedingObligations = [];
    }
    game.discardPile.push(card);
    pushLog(game, `${player.name} satisfied ${cleared} feeding obligation${cleared === 1 ? "" : "s"} with ${card.name}.`);
    return finishCardEffect(game, options);
  }
  if (Number(card.number) === 292) {
    if (player.bunnies.length < 2) {
      discardUsedWeapon(game, card);
      pushLog(game, `${player.name} could not crew ${card.name} with two bunnies.`);
      return finishCardEffect(game, options);
    }
    game.phase = "f18Crew";
    game.pendingAction = { playerIndex, effect: "f18Crew", card, ...effectReturnState(options) };
    game.message = "Choose exactly two of your bunnies to crew the F-18 Hornet.";
    return game;
  }
  if (card.kind === "weapon" || card.kind === "feed") {
    const badKarma = card.kind === "weapon" && player.badKarma;
    const hasTarget = badKarma ? player.bunnies.length > 0 : Boolean(findBotTarget(game, playerIndex, card));
    if (!hasTarget) {
      discardUsedWeapon(game, card);
  ...
  if (card.kind === "shopMarket") {
    ...
    if (shop) {
      ...
      if (Number.isFinite(card.defensePrice)) shop.defensePrice = card.defensePrice;
      if (card.weaponPriceMode) shop.weaponPriceMode = card.weaponPriceMode;
      if (Number.isFinite(card.pawnPrice)) shop.pawnPrice = card.pawnPrice;
      if (Number.isFinite(card.bunnyPrice)) shop.bunnyPrice = card.bunnyPrice;
...
function continueBlueRewards(game, pending) {
  ...
  if ((pending.carrotCount || 0) > 0 && getKaballasMarket(game).isOpen && game.carrotMarket.length) {
    game.phase = "chooseCarrot";
    game.pendingAction = { ...pending, playerIndex, effect: "chooseCarrot", remainingCount: Math.min(pending.carrotCount, game.carrotMarket.length), blueRewards: true };
...
function isAreaWeapon(card) {
  return [48, 131, 132, 133, 182, 236, 237, 294].includes(Number(card?.number));
}
...
function continueAreaWeapon(game) {
  ...
  game.phase = "areaWeaponRoll";
  game.message = `${game.players[current.playerIndex].name}: roll a d${Number(current.power) >= 13 ? 20 : 12} for ${current.name} against ${pending.card.name} level ${current.power}.`;
  return game;
}
...
function beginMagicReveal(game) {
  if (!game.winningZodiac && game.zodiacDeck?.length) {
    game.winningZodiac = game.zodiacDeck.pop();
    const holderIndexes = game.players.map((player, index) => (player.zodiacCards || []).some((card) => card.sign === game.winningZodiac.sign) ? index : null).filter(Number.isInteger);
    pushLog(game, `${game.winningZodiac.sign} is the Winning Zodiac${holderIndexes.length ? `; held by ${holderIndexes.map((index) => game.players[index].name).join(" and ")}` : ""}.`);
    if (holderIndexes.length) {
      game.phase = "zodiacPrivilege";
      game.pendingAction = { playerIndex: holderIndexes[0], effect: "winningZodiac", holderQueue: holderIndexes };
      game.message = `${game.players[holderIndexes[0]].name}: you may move any one bunny to any other player before the Magic Carrot reveal.`;
      return game;
    }
  }
  game.phase = "reveal";
...
function resolveWinningZodiacPrivilege(game, pending, action) {
  const holderIndex = pending.playerIndex;
  const holder = game.players[holderIndex];
  if (action.bunnyId) {
    const sourceIndex = game.players.findIndex((player) => player.bunnies.some((bunny) => bunny.id === action.bunnyId));
    const targetIndex = Number(action.targetPlayerIndex);
    if (sourceIndex < 0 || !game.players[targetIndex] || sourceIndex === targetIndex) throw new Error("Move one bunny to a different player, or pass.");
    const source = game.players[sourceIndex];
    const [bunny] = source.bunnies.splice(source.bunnies.findIndex((entry) => entry.id === action.bunnyId), 1);
    const obligations = (source.feedingObligations || []).filter((entry) => entry.bunnyId === bunny.id);
    source.feedingObligations = (source.feedingObligations || []).filter((entry) => entry.bunnyId !== bunny.id);
    game.players[targetIndex].bunnies.push(bunny);
    game.players[targetIndex].feedingObligations.push(...obligations);
    pushLog(game, `${holder.name} moved ${bunny.name} from ${source.name} to ${game.players[targetIndex].name} with the Winning Zodiac privilege.`);
  }

  const sign = game.winningZodiac.sign;
  const birthMatch = holder.birthZodiac === sign;
  const currentMatch = currentZodiacSign() === sign;
  if (birthMatch && currentMatch) {
    let taken = 0;
    for (const [index, opponent] of game.players.entries()) {
      if (index === holderIndex || opponent.carrots.length <= 1) continue;
      const transfer = opponent.carrots.splice(1);
      taken += transfer.length;
      holder.carrots.push(...transfer);
    }
    pushLog(game, `${holder.name} matched both birth and current Zodiac and took ${taken} Carrots, leaving each opponent one.`);
  } else if (birthMatch || currentMatch) {
    const richestCount = Math.max(0, ...game.players.map((player, index) => index === holderIndex ? -1 : player.carrots.length));
    const eligible = game.players.map((player, index) => ({ player, index })).filter(({ player, index }) => index !== holderIndex && player.carrots.length === richestCount);
    const selected = eligible.find(({ index }) => index === Number(action.carrotSourcePlayerIndex)) || eligible[0];
    if (selected && richestCount > 0) {
      const transfer = selected.player.carrots.splice(0, Math.min(3, selected.player.carrots.length));
      holder.carrots.push(...transfer);
      pushLog(game, `${holder.name} matched ${birthMatch ? "their birth" : "the current"} Zodiac and took ${transfer.length} Carrots from ${selected.player.name}.`);
    }
  }

  const queue = pending.holderQueue.filter((index) => index !== holderIndex);
  if (queue.length) {
    game.pendingAction = { ...pending, playerIndex: queue[0], holderQueue: queue };
    game.message = `${game.players[queue[0]].name}: you may move any one bunny to any other player before the Magic Carrot reveal.`;
    return game;
  }
  game.phase = "reveal";
  game.pendingAction = null;
  game.message = "Winning Zodiac privileges are complete. Click the hidden Magic Carrot pile to reveal the winner!";
  return game;
}

function settleMagicCarrot(game, magic) {
  const hasLivingBunny = (player, index) => player.bunnies.some(isLivingBunny)
    || (game.area51Abducted?.ownerIndex === index
      && (game.area51Abducted.bunnies || [game.area51Abducted.bunny]).filter(Boolean).some(isLivingBunny));
...
function dealPlayableCard(game, player, playerIndex, random = Math.random) {
  const card = drawMain(game);
  if (!card) return;
  if (card.kind === "money" && card.randomValue) {
    const value = rollDie(20, random);
    player.dollaCredit = (player.dollaCredit || 0) + value;
    game.discardPile.push(card);
    recordDiceRoll(game, [{ value, sides: 20, color: "clear" }], card.name, player.name);
  } else if (card.kind === "money") player.bank.push(card);
  else if (card.kind === "zodiac") {
    player.zodiacCards ||= [];
    player.zodiacCards.push(card);
  }
...
function beginImmediateCard(game, playerIndex, card, returnState = {}) {
  ...
  if (Number(card.number) === 365) {
    const queue = Array.from({ length: game.players.length }, (_, offset) => (playerIndex + offset * (game.turnDirection || 1) + game.players.length) % game.players.length)
      .filter((index) => (game.players[index].zodiacCards || []).length);
    if (!getKaballasMarket(game).isOpen || !game.carrotMarket.length || !queue.length) {
      game.discardPile.push(card);
      pushLog(game, `${card.name} found no eligible Zodiac holder or an open Carrot market.`);
      return finishImmediateEffect(game, returnState);
    }
    game.phase = "chooseCarrot";
    game.pendingAction = { playerIndex: queue[0], drawingPlayerIndex: playerIndex, effect: "communityCarrot", card, communityCarrotQueue: queue, ...returnState };
    game.message = `${game.players[queue[0]].name}: choose your Community Carrot.`;
    return game;
  }
...
function drawMain(game) {
  if (!game.mainDeck.length && (game.discardPile.length || game.rooneysEmporium?.weaponDiscard.length || game.weilsPawnShop?.bunnyDiscard.length)) {
    ...
    const recyclable = game.discardPile.filter((card) => ["bunny", "weapon", "feed", "chooseCarrot", "defense", "special", "verySpecial", "money", "zodiac", "action", "modifier", "misfortune", "market", "shopMarket"].includes(card.kind));
...
function isWeilsCouponActive(game, playerIndex) {
  return Number.isInteger(playerIndex)
    && game?.players?.[playerIndex]?.weilsCouponTurnNumber === game.turnNumber
    && game.currentPlayerIndex === playerIndex;
}
...
function availableStoreDolla(game, playerIndex) {
  const player = game.players[playerIndex];
  const greenCredit = player.greenStoreCreditTurnNumber === game.turnNumber ? (player.greenStoreCredit || 0) : 0;
  return bankTotal(player) + redGleefulStoreCredit(game, playerIndex) + greenCredit;
}
...
function spendStoreDolla(game, playerIndex, cost) {
  ...
  const greenCredit = player.greenStoreCreditTurnNumber === game.turnNumber
    ? Math.min(player.greenStoreCredit || 0, cost) : 0;
  player.greenStoreCredit = Math.max(0, (player.greenStoreCredit || 0) - greenCredit);
  const remainingAfterGreen = cost - greenCredit;
  const credit = Math.min(redGleefulStoreCredit(game, playerIndex), remainingAfterGreen);
  if (credit) {
    player.redGleefulCreditTurnNumber = game.turnNumber;
    player.redGleefulCreditSpent = (player.redGleefulCreditSpent || 0) + credit;
  }
  if (remainingAfterGreen > credit) spendBank(player, remainingAfterGreen - credit, game.discardPile);
}

function canShopAfterRun(game, playerIndex, shop) {
  const player = game?.players?.[playerIndex];
  if (!player || game.currentPlayerIndex !== playerIndex || game.phase !== "draw") return false;
  if (player.greenStoreCreditTurnNumber === game.turnNumber && (player.greenStoreCredit || 0) > 0) return true;
  if (shop === "kaballas") return isKaballasCouponActive(game, playerIndex) || player.kaballasGiftTurnNumber === game.turnNumber;
  if (shop === "rooneys") return isRooneysCouponActive(game, playerIndex);
  if (shop === "weils") return isWeilsCouponActive(game, playerIndex);
  return false;
}
...
function discardBunny(game, bunny, killerPlayerIndex = null) {
  const cuffs = (game.fingercuffs || []).find((entry) => entry.bunnyIds.includes(bunny.id));
  if (cuffs) {
    game.fingercuffs = game.fingercuffs.filter((entry) => entry.id !== cuffs.id);
    game.discardPile.push(cuffs.card);
    const partnerId = cuffs.bunnyIds.find((id) => id !== bunny.id);
    const partner = findKillerBunnyInCircle(game, partnerId);
    if (partner) {
      partner.player.bunnies.splice(partner.bunnyIndex, 1);
      pushLog(game, `${partner.bunny.name} was eliminated with ${bunny.name} because they were joined by Fingercuffs.`);
      discardBunny(game, partner.bunny, killerPlayerIndex);
    }
  }
  removeKillerBunnyFromCircle(game, bunny.id);
  if (bunny.bounty) {
...
function getEffectiveCloverReduction(game, bunny) {
  const own = getKillerBunniesCloverReduction(bunny);
  const cuffs = (game.fingercuffs || []).find((entry) => entry.bunnyIds.includes(bunny.id));
  if (!cuffs) return own;
  const partnerId = cuffs.bunnyIds.find((id) => id !== bunny.id);
  const partner = findKillerBunnyInCircle(game, partnerId)?.bunny;
  return own + (partner ? getKillerBunniesCloverReduction(partner) : 0);
}
...
function hasAlbinoProtection(game, playerIndex, card) {
  if (![182, 237, 294].includes(Number(card?.number))) return false;
  return game.players.some((player, ownerIndex) => (player.savedSpecials || []).some((special) => Number(special.number) === 362
    && (special.protectedPlayerIndex ?? ownerIndex) === playerIndex));
}
...
function chooseBotCardAction(game, playerIndex) {
  ...
  switch (game.phase) {
    ...
    case "lowJackRoll":
    case "pilferPawnRoll":
    case "randomFeedRoll": return {};
    case "dayDeadRoll": return {};
    case "dayDeadRevive": return { bunnyIds: (pending.eligibleBunnyIds || []).slice(0, pending.maxRevive || 3) };
    case "fingercuffs": return { bunnyIds: getKillerBunniesCircleEntries(game).slice(0, 2).map((entry) => entry.bunny.id) };
    case "precessionBirth": return { birthZodiac: game.players[playerIndex].birthZodiac || "Aries" };
    case "laTapeWorm": return { obligationId: pending.obligations?.[0]?.id };
    case "zodiacResetRun": return { cardIds: game.players[playerIndex].hand.slice(0, 2).map((card) => card.id) };
    case "leifTarget": return { targetPlayerIndex: pending.targetIndexes?.[0] };
    case "leifRoll": return {};
    case "leifPassenger": return { bunnyId: game.leifCarrotson?.passengers[0]?.id };
    case "spoilsportTarget": return { bunnyIds: game.players[pending.targetPlayerIndex]?.bunnies.slice(0, pending.maxKills || 1).map((bunny) => bunny.id) || [] };
    case "albinoTarget": return { targetPlayerIndex: playerIndex };
    case "zodiacPrivilege": {
      const bunny = game.players.flatMap((player, sourcePlayerIndex) => player.bunnies.map((entry) => ({ bunny: entry, sourcePlayerIndex })))[0];
      const targetPlayerIndex = bunny ? game.players.findIndex((_, index) => index !== bunny.sourcePlayerIndex) : -1;
      return bunny ? { bunnyId: bunny.bunny.id, targetPlayerIndex } : {};
    }
    case "bunnyHop": return { leftPlayerIndex: playerIndex };
    case "coolChange": return { cardIds: [] };
    case "laHotPeppers": return { obligationId: pending.obligations?.[0]?.id };
    case "paintballTarget": {
      const target = game.players.flatMap((player, targetPlayerIndex) => player.bunnies.map((item) => ({ item, targetPlayerIndex })))[0];
      return { targetPlayerIndex: target?.targetPlayerIndex, itemId: target?.item.id, itemType: "bunny" };
    }
    case "runAmokTarget": return { targetPlayerIndex: game.players.findIndex((_, index) => index !== playerIndex) };
    case "runTransformer": return { cardId: pending.eligibleIds?.[0] };
    case "freePawnChoice": return { pawnIds: game.weilsPawnShop?.pawnSupply.slice(0, 2).map((pawn) => pawn.id) || [] };
    case "royaleGamble": return { choice: "keep" };
    case "f18Crew": return { bunnyIds: game.players[playerIndex].bunnies.slice(0, 2).map((bunny) => bunny.id) };
    case "carrotExchange": {
...
function recordDiceRoll(game, dice, label, rollerName = game.players?.[game.pendingAction?.playerIndex]?.name || "Player") {
  const normalized = dice.map((die) => ({
    value: Number.isFinite(Number(die.value)) ? Number(die.value) : String(die.value),
    sides: Number(die.sides),
    color: die.color || null,
  }));
  const uniformSides = normalized.every((die) => die.sides === normalized[0].sides) ? normalized[0].sides : "mixed";
  game.rollSequence = (game.rollSequence || 0) + 1;
  game.lastRoll = {
    id: game.rollSequence,
    dice: normalized,
    value: normalized.length === 1
      ? normalized[0].value
      : normalized.map((die) => `${die.color ? `${capitalize(die.color)} ` : ""}d${die.sides}: ${die.value}`).join(" · "),
    sides: uniformSides,
    ...(normalized.length === 1 && normalized[0].color ? { color: normalized[0].color } : {}),
    label,
    rollerName,
  };
...
function findBotTarget(game, playerIndex, card = null) {
  for (let offset = 1; offset < game.players.length; offset += 1) {
    const index = (playerIndex + offset) % game.players.length;
    const bunny = game.players[index].bunnies.find((entry) => !isIntangibleHologram(entry) || isHolographicVulnerableTo(card));
    if (bunny) return { playerIndex: index, bunny };
  }
  return null;
}
