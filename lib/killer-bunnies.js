...
function resolveKillerBunniesDefenseCore(game, playerIndex, choice, random = Math.random) {
  ...
  if (pending.effect === "weapon") {
    ...
    if (choice === "defense") {
      ...
      recordDiceRoll(next, [{ value: roll, sides, color: sides === 20 ? "clear" : null }], pending.card.name);
      if (target.shields > 0) {
        target.shields -= 1;
    ...
    if (weaponFailed && Number(pending.card.number) === 287) {
      ...
      if (nextBunny) {
        const reduction = getKillerBunniesCloverReduction(nextBunny);
        next.phase = "defend";
        next.pendingAction = {
          ...pending,
...
export function getKillerBunniesShopItemStatus(game, playerIndex, shop, item, cardId) {
  ...
  const couponShoppingAfterRun = game.phase === "draw" && (shop === "rooneys" ? isRooneysCouponActive(game, playerIndex) : isWeilsCouponActive(game, playerIndex));
