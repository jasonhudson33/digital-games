...
export function gainMoney(game, playerId, workerId, cardIndex) {
  ...
  if (!found || !workerAdjacentCardIndexes(game, found.slotId, found.worker).includes(Number(cardIndex))) return game;
  const player = currentPlayer(game);
...
export function runComputerTurn(game, rng = Math.random) {
  ...
  if (worker) {
    ...
    const cardChoices = workerAdjacentCardIndexes(game, found.slotId, found.worker);
