...
export const RESIDENCE_VALUES = [2, 3, 4, 5, 6, 7];
...
export function createPeriodDecks(rng = Math.random) {
  ...
  for (const spec of CARD_SPECS) {
    for (let copy = 1; copy <= spec.copies; copy += 1) {
      decks[spec.period].push({ ...spec, id: `${spec.period}-${spec.slug}-${copy}`, tokens: [] });
    }
  }
...
export function adjacentSlotsForCard(cardIndex) {
  return marketSlots().filter((slot) => slot.cards.includes(Number(cardIndex))).map((slot) => slot.id);
}

export function congestionForCard(game, cardIndex, excludedWorkerId = null) {
  return adjacentSlotsForCard(cardIndex).reduce((total, slotId) => total + game.workerSlots[slotId].filter((worker) => worker.id !== excludedWorkerId).length, 0);
}
...
export function placeWorker(game, playerId, slotId) {
  ...
  const worker = { id: `${playerId}-${game.round}-${player.totalWorkers - player.activeWorkers + 1}-${Date.now()}`, playerId };
...
export function activationCost(game, playerId, workerId, cardIndex, options = {}) {
  ...
  const card = game.market[cardIndex];
  const found = findWorker(game, playerId, workerId);
  if (!player || !card || !found || !adjacentCardIndexes(found.slotId).includes(Number(cardIndex))) return null;
...
export function activateMarketCard(game, playerId, workerId, cardIndex, options = {}) {
  ...
  const found = findWorker(game, playerId, workerId);
  const card = game.market[cardIndex];
  if (!found || !card || !adjacentCardIndexes(found.slotId).includes(Number(cardIndex))) return game;
...
export function useEvent(game, playerId, options = {}) {
  ...
  if (event.id === "windfall") {
    ...
    const worker = { id: `${playerId}-${game.round}-late-${Date.now()}`, playerId };
...
export function runComputerTurn(game, rng = Math.random) {
  ...
  if (player.phase === "placement") {
    if (player.activeWorkers > 0 && rng() > 0.25) {
      ...
      return placeWorker(game, player.id, legal[Math.floor(rng() * legal.length)].id);
    }
    return beginActivation(game, player.id);
  }
  const worker = firstOwnedWorker(game, player.id);
  if (worker) {
    const found = findWorker(game, player.id, worker.id);
    const cardChoices = adjacentCardIndexes(found.slotId).filter((index) => game.market[index]);
    const affordable = cardChoices.map((index) => ({ index, cost: activationCost(game, player.id, worker.id, index) })).filter((choice) => choice.cost && choice.cost.total <= player.money && (!game.market[choice.index].token || game.market[choice.index].tokens.length));
    if (affordable.length) {
      affordable.sort((a, b) => (game.market[b.index].points || 0) - (game.market[a.index].points || 0) || a.cost.total - b.cost.total);
      const choice = affordable[0];
      return activateMarketCard(game, player.id, worker.id, choice.index, { residenceChoice: "advance", tokenValue: game.market[choice.index].tokens?.[0] });
    }
    const cardIndex = cardChoices[0] ?? adjacentCardIndexes(found.slotId)[0];
    return gainMoney(game, player.id, worker.id, cardIndex);
  }
  ...
  if (usable && rng() > 0.35) {
    ...
    return useBuilding(game, player.id, usable.id, index);
  }
  if (!player.eventUsed) {
    const options = { resource: "score", residenceChoice: "advance", spend: player.money >= 6 ? 6 : 3, slotId: marketSlots().find((slot) => slot.cards.some((index) => game.market[index]))?.id, buildingId: player.buildings.find((building) => building.used)?.id };
    const eventResult = useEvent(game, player.id, options);
    if (eventResult !== game) return eventResult;
  }
  return pass(game, player.id);
}
