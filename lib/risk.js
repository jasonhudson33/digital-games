...
/*
 * Player colours, built on a lightness ladder rather than a hue wheel.
 *
 * Six hues cannot be made safe by hue alone: deuteranopia and protanopia both
 * collapse the red-green axis, and the old set lost four pairs to it — red and
 * green sat 26 apart, blue and purple 16, where under about 40 reads as the same
 * colour. Lightness survives every form of colour blindness, so these six are
 * spread from OKLCH L=0.36 to L=0.78 across six hues. The closest pair under any
 * of the three dichromacies is now 69.
 *
 * Every colour also takes its ink at 4.65:1 or better, which matters because the
 * army count sits on it. On the old palette not one of the six reached 4.5:1 and
 * the human player's gold was 1.76:1.
 *
 * `emblem` is the second channel — see lib/risk-emblems.js.
 */
export const PLAYER_STYLES = [
  { name: "sand", color: "#dfad76", emblem: "star" },
  { name: "clay", color: "#b76c62", emblem: "crown" },
  { name: "aqua", color: "#47aeb9", emblem: "anchor" },
  { name: "slate", color: "#5268a6", emblem: "shield" },
  { name: "plum", color: "#5b2457", emblem: "diamond" },
  { name: "olive", color: "#3d5f1a", emblem: "leaf" },
];

export const PLAYER_COLORS = {
  human: PLAYER_STYLES[0].color,
  bot1: PLAYER_STYLES[1].color,
  bot2: PLAYER_STYLES[3].color,
};
...
export function createRiskGameForPlayers(inputPlayers, rng = Math.random, room = {}) {
  ...
  const openingReinforcements = calculateReinforcements(territories, players[startingPlayerIndex].id);
  return {
    ...room,
    ...
    tradesCompleted: 0,
    log: [
      `${players[startingPlayerIndex].name} begins with ${openingReinforcements} armies to deploy.`,
...
export function mustTradeCards(game, playerId = game.players[game.currentPlayerIndex]?.id) {
  const player = game.players.find((item) => item.id === playerId);
  return Boolean(player?.cards?.length >= 5 && findTradeSet(player.cards));
}

export function tradeCards(game, playerId = game.players[game.currentPlayerIndex]?.id) {
  const currentPlayer = game.players[game.currentPlayerIndex];
  if (currentPlayer?.id !== playerId || game.pendingConquest) return game;
  const player = game.players.find((item) => item.id === playerId);
  const mandatory = mustTradeCards(game, playerId);
  if (game.phase !== "reinforce" && !mandatory) return game;
  ...
  const interruptedPhase = game.phase !== "reinforce" ? game.phase : game.resumePhaseAfterTrade;
  return {
    ...game,
    phase: "reinforce",
    resumePhaseAfterTrade: interruptedPhase ?? null,
...
export function placeReinforcement(game, territoryId, count = 1) {
  const currentPlayer = game.players[game.currentPlayerIndex];
  if (game.winnerId || game.phase !== "reinforce" || mustTradeCards(game) || count < 1 || count > game.reinforcements) return game;
...
export function advancePhase(game) {
  if (game.winnerId || game.pendingConquest || mustTradeCards(game)) return game;
  if (game.phase === "reinforce") {
    if (game.reinforcements > 0) return game;
    const nextPhase = game.resumePhaseAfterTrade ?? "attack";
    return {
      ...game,
      phase: nextPhase,
      resumePhaseAfterTrade: null,
      log: [nextPhase === "attack"
        ? "Attack phase — select one of your territories, then an adjacent rival."
        : "Fortify phase — move armies once, or end your turn.", ...game.log],
    };
  }
...
export function beginNextTurn(game, rng = Math.random) {
  ...
  const startingPlayerIndex = rewardedGame.startingPlayerIndex ?? 0;
  const currentOffset = (rewardedGame.currentPlayerIndex - startingPlayerIndex + rewardedGame.players.length) % rewardedGame.players.length;
  const nextOffset = (nextIndex - startingPlayerIndex + rewardedGame.players.length) % rewardedGame.players.length;
  const wrapped = nextOffset <= currentOffset;
...
export function runComputerTurn(game, rng = Math.random) {
  ...
  for (let attacks = 0; attacks < 18 && !next.winnerId; attacks += 1) {
    ...
    while (mustTradeCards(next, player.id)) next = tradeCards(next, player.id);
    while (next.phase === "reinforce" && next.reinforcements > 0) {
      const target = borderTerritories(next, player.id)[0] ?? territoriesForPlayer(next.territories, player.id)[0];
      next = placeReinforcement(next, target, 1);
    }
    if (next.phase === "reinforce") next = advancePhase(next);
  }
