...
export const shuffleScumDeck = shuffled;
...
export function createScumGame({
  ...
  const normalizedStartingPlayerIndex = ((startingPlayerIndex % normalizedPlayerCount) + normalizedPlayerCount) % normalizedPlayerCount;
  const requestedDealer = dealerIndex === null
    ? (normalizedStartingPlayerIndex - 1 + normalizedPlayerCount) % normalizedPlayerCount
    : dealerIndex;
  ...
  return {
    ...
    roundNumber: 1,
    startingPlayerIndex: openingPlayerIndex,
    turnOrder,
...
export function chooseScumBotPlay(state, playerIndex) {
  ...
  return [...legal].sort((left, right) => (
    scoreScumBotPlay(right, hand, state) - scoreScumBotPlay(left, hand, state)
    || getScumPlayRank(left) - getScumPlayRank(right)
  ))[0];
}

function scoreScumBotPlay(cards, hand, state) {
  const rank = getScumPlayRank(cards);
  const jokerCount = cards.filter(isJoker).length;
  const remaining = hand.length - cards.length;
  if (remaining === 0) return 1_000_000;
  const burnsPile = cards.length === 4 && cards.every((card) => isJoker(card) || card.rank === rank);
  const controlPenalty = rank >= 14 ? (14 - Math.min(remaining, 10)) * 8 : 0;
  const jokerPenalty = jokerCount * 140;
  const sizeValue = cards.length * (state.pile ? 35 : 70);
  const rankCost = rank * (state.pile ? 5 : 2);
  return (burnsPile ? 600 : 0) + sizeValue - rankCost - jokerPenalty - controlPenalty;
}
