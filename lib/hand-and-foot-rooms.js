...
export async function joinHandFootRoom({ roomCode, name }) {
  ...
  if (room.players.length >= HAND_FOOT_MAX_PLAYERS) throw new Error("That room already has sixteen players.");
...
export async function addHandFootComputer({ roomCode, token }) {
  ...
  requireLobby(room);
  if (room.players.length >= HAND_FOOT_MAX_PLAYERS) throw new Error("Hand and Foot supports at most sixteen players.");
...
export async function startHandFootRoom({ roomCode, token, random = Math.random }) {
  const room = await requireRoom(roomCode);
  requireHost(room, token);
  requireLobby(room);
...
export async function drawHandFootRoomCards({ roomCode, token, pileIndex = null, cardCount = null }) {
  return updateGame(roomCode, token, (game, playerIndex) => drawHandFootCards(game, playerIndex, pileIndex, cardCount));
}

export async function playHandFootRoomCards({ roomCode, token, cardIds, targetRank = null }) {
  return updateGame(roomCode, token, (game, playerIndex) => playHandFootCards(game, playerIndex, cardIds, targetRank));
}
...
export function buildHandFootTeamPairs(players, teammatePreferences = {}) {
  if (players.length < 4 || players.length % 2 !== 0 || players.length > HAND_FOOT_MAX_PLAYERS) {
...
function runHandFootComputers(room) {
  ...
  while (room.game?.phase === "playing" && turns < 1000) {
    ...
    while (next.phase === "playing" && meldAttempts < 12) {
      meldAttempts += 1;
      const move = chooseHandFootBotMove(next, playerIndex);
      if (!move?.cardIds.length) break;
      try {
        next = playHandFootCards(next, playerIndex, move.cardIds, move.targetRank);
      } catch {
        break;
      }
    }
...
function serializeRoom(room, token) {
  ...
  return {
    ...
    drawPiles: (game.drawPiles || [game.drawPile]).map((pile) => Array(pile.length).fill(null)),
    players: game.players.map((player, index) => ({
      ...player,
      handCount: player.hand.length,
      footCount: player.foot.length,
      pendingDrawCount: (player.pendingDraw || []).length,
      hand: index === viewerPlayerIndex ? player.hand : [],
      foot: index === viewerPlayerIndex && player.usingFoot ? player.foot : [],
      pendingDraw: index === viewerPlayerIndex ? (player.pendingDraw || []) : [],
