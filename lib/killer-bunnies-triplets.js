...
export function getKillerBunniesExtraRunStatus(player) {
  ...
  for (const bunny of bunnies) {
    ...
    if (profile.color) {
      addCount(colorCounts, profile.color, profile.units);
      if (!profile.robot) addCount(nonRobotColorCounts, profile.color, profile.units);
    }
...
function bunnyTripletProfile(bunny) {
  ...
  const family = /\bspecialty bunny\b/i.test(name) ? "specialty"
  ...
  const color = normalized === "enginerds" ? "green"
    : /holographic bunny/i.test(name) ? "red"
      : String(bunny?.color || "").toLowerCase() || null;
  const units = /\bquintet\b/i.test(name) ? 5
    : /\btriple\b/i.test(name) ? 3
      : /\bdouble\b/i.test(name) || normalized === "enginerds" ? 2 : 1;
  return { color, family, kind, robot, units, wild };
}
