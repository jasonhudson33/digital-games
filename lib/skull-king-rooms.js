...
export async function startSkullKingRoom({ roomCode, token }) {
  const room = await requireRoom(roomCode);
  requireHost(room, token);
  requireLobby(room);
  ...
  room.game = createSkullKingMatch({ playerSeeds: room.players });
...
export async function restartSkullKingRoom({ roomCode, token }) {
  const room = await requireRoom(roomCode);
  requireHost(room, token);
