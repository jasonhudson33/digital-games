...
const ACTIONS = [
  ["You publish a bestseller!", 40000, "📖"],
  ["A road trip becomes an epic memory.", -20000, "🚐"],
  ["You win the neighborhood bake-off!", 30000, "🧁"],
  ["Your car needs a heroic repair.", -30000, "🛠️"],
  ["You launch a clever side hustle!", 50000, "💡"],
  ["A dream vacation calls your name.", -40000, "🏖️"],
  ["You find a rare treasure at a yard sale!", 60000, "💎"],
  ["You host the party of the year.", -20000, "🎉"],
  ["Your garden wins a city prize!", 30000, "🌻"],
  ["You sail around the world.", -50000, "⛵"],
];
...
const blueprint = [
  ...
  ["payday", "Payday", "💵"],
  ["action", "A new adventure", "✨"],
  ["life", "Help your community", "🤝"],
  ["action", "Make a memory", "📸"],
  ["payday", "Payday", "💵"],
  ["action", "Life happens", "🎈"],
  ["job", "Career choice", "💼"],
  ["life", "Volunteer day", "🌱"],
  ...
  ["life", "Community hero", "🦸"],
  ["payday", "Payday", "💵"],
  ["action", "One more adventure", "🗻"],
  ["tax", "Taxes due", "🧮"],
  ["life", "Dream achieved", "🌟"],
  ["payday", "Payday", "💵"],
  ["action", "The home stretch", "🏁"],
  ["career", "Career event", "💼"],
  ["life", "Leave a legacy", "🌈"],
  ["payday", "Final payday", "💵"],
  ["retire", "Retirement", "🌴"],
...
export const BOARD = blueprint.map(([type, label, icon], index) => ({ index, type, label, icon }));
...
export function createLifeLobby(host, roomCode) {
  return {
    ...
    pending: null,
    winnerIds: [],
    lifeTilePool: shuffle([...LIFE_TILES, ...LIFE_TILES]),
    millionaireBonus: shuffle([...LIFE_TILES]).slice(0, 4),
...
export function spinLife(state, playerId, forcedSpin) {
  ...
  next = logState(next, `${player.name} spun ${spin} and moved to ${BOARD[target].label}.`);
...
function resolveSpace(state, playerId, space) {
  ...
  if (space.type === "action") {
    const [label, amount, icon] = ACTIONS[Math.floor(Math.random() * ACTIONS.length)];
...
function randomCareers(allowDegree, count) {
  return shuffle(CAREERS.filter((career) => allowDegree || !career.degree)).slice(0, count);
}
