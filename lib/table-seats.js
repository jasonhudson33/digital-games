...
export const WIDE_BUSY = { ...WIDE_TABLE, card: { w: 54, h: 75 }, gap: 14 };
...
const pick = (count, roomy, busy, crowded) => (
  count > CROWDED_ABOVE ? crowded : count > BUSY_ABOVE ? busy : roomy
);
 ...
 * @returns {Array<{key: string, tier: string, shape: object}>} `key` is the
 ...
 *          `tier` is "roomy", "busy" or "crowded" and goes on the felt as a class,
 *          so the furniture drawn is the furniture that was solved for.
 */
export function tableShapes(count, table = {}) {
  const tier = pick(count, "roomy", "busy", "crowded");
  return [
    { key: "", tier, shape: { ...pick(count, WIDE_TABLE, WIDE_BUSY, WIDE_CROWD), ...table.wide } },
    { key: "-n", tier, shape: { ...pick(count, PHONE_TABLE, PHONE_TABLE, PHONE_CROWD), ...table.phone } },
  ];
}

/**
 * Each card is laid down with a small lean, so a trick looks played rather than
 * pasted on. A leaning card covers a bigger box than an upright one, and the
 * table has to solve against the box it actually covers — at four degrees a
 * phone-sized card gains four pixels of width, which is most of the gap between
 * that card and the name plate beside it.
 */
export const MAX_TILT = 4;

export const leaning = ({ w, h }, degrees = MAX_TILT) => {
  const radians = (Math.abs(degrees) * Math.PI) / 180;
  return {
    w: (w * Math.cos(radians)) + (h * Math.sin(radians)),
    h: (w * Math.sin(radians)) + (h * Math.cos(radians)),
  };
};
...
export function seatLayout(count, viewer, shape = WIDE_TABLE) {
  const { box, plate, centre, gap } = { ...WIDE_TABLE, ...shape };
  const card = leaning({ ...WIDE_TABLE, ...shape }.card);
