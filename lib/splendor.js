...
/*
 * Every cost, bonus and noble requirement on this board is read off a small
 * coloured disc with a number in it, so these six colours have to do two jobs at
 * once: hold legible text, and stay apart from each other.
 *
 * The old set did neither. White on gold was 2.21:1, on emerald 3.77:1, on ruby
 * 4.46:1 — three of six below the 4.5:1 floor. And emerald and ruby sat 25 apart
 * under deuteranopia, sapphire and emerald 28 apart under tritanopia, where
 * under about 45 reads as the same colour.
 *
 * These are chosen at each gem's own hue, with lightness and chroma solved so
 * that every gem takes its ink at 5.9:1 or better and the closest pair under any
 * of the three dichromacies is 75. `short` is the second channel — the letter is
 * drawn on the disc, so the board also works in greyscale.
 */
export const GEM_INFO = {
  white: { name: "Diamond", short: "D", hex: "#ede4ca", ink: "#181d22" },
  blue: { name: "Sapphire", short: "S", hex: "#005eae", ink: "#fffdf7" },
  green: { name: "Emerald", short: "E", hex: "#00af6a", ink: "#181d22" },
  red: { name: "Ruby", short: "R", hex: "#a13835", ink: "#fffdf7" },
  black: { name: "Onyx", short: "O", hex: "#181a21", ink: "#fffdf7" },
  gold: { name: "Gold", short: "G", hex: "#d09400", ink: "#181d22" },
};
...
const CLASSIC_CARD_DATA = {
  ...
  white: [
    ...
    [0, 2, 0, 0, 2, 0], [0, 1, 0, 1, 1, 2], [0, 1, 3, 0, 1, 0], [1, 0, 0, 0, 0, 4],
...
/*
 * The replacement goes into the slot that was vacated.
 *
 * Appending it to the end of the row made every survivor shift left, so the card
 * a player was about to click moved the moment somebody bought something — and
 * any index captured before the move pointed at a different card afterwards. On
 * a table the new card simply fills the gap.
 */
function refillMarket(game, level, market, slot) {
  const deck = game.decks[level];
  if (!deck.length) return { decks: game.decks, market };
  const row = [...market[level]];
  row.splice(Math.min(Math.max(0, slot ?? row.length), row.length), 0, deck[0]);
  return {
    decks: { ...game.decks, [level]: deck.slice(1) },
    market: { ...market, [level]: row },
  };
}
...
export function reserveCard(game, playerId, source) {
  ...
  if (source?.kind === "deck") {
    ...
    const refilled = refillMarket({ ...game, decks }, level, market, index);
...
/**
 * Can this player do anything at all?
 *
 * A player can reach a position with no legal action: the gem piles empty, three
 * cards already reserved, and nothing affordable. The printed rules let them
 * pass; without one the turn cannot advance, and a bot in that position returned
 * the same state forever. Six per cent of randomly played games reach it.
 */
export function hasLegalMove(game, playerId) {
  const player = game.players.find((item) => item.id === playerId);
  if (!player) return false;
  if (GEM_COLORS.some((color) => game.bank[color] > 0)) return true;
  if (player.reserved.length < 3 && [1, 2, 3].some((level) => game.decks[level].length || game.market[level].length)) return true;
  const buyable = [...player.reserved, ...[1, 2, 3].flatMap((level) => game.market[level])];
  return buyable.some((card) => canPurchase(player, card));
}

/**
 * Pass. Only legal when there is genuinely nothing else to do, so it can never
 * be used to skip a turn.
 */
export function passTurn(game, playerId) {
  if (!canAct(game, playerId)) return game;
  if (hasLegalMove(game, playerId)) return game;
  const player = currentPlayer(game);
  return endAction({ ...game, log: [`${player.name} could not act and passed.`, ...game.log].slice(0, 60) }, playerId);
}
...
export function runComputerTurn(game, rng = Math.random) {
  ...
  if (bot.reserved.length < 3) {
    ...
    // The market can be empty while a deck still has cards in it. Reserving
    // blind is a real move and the bot never used to try it, which left it one
    // escape short of a human in exactly the position where it mattered.
    const level = [3, 2, 1].find((candidate) => game.decks[candidate].length);
    if (level) return reserveCard(game, bot.id, { kind: "deck", level });
  }

  // Nothing legal is left. Pass, so the turn advances — returning the same state
  // here is what made `while (isComputer) game = runComputerTurn(game)` hang.
  return passTurn(game, bot.id);
}
