...
function advanceBidding(game, afterPlayerIndex) {
  ...
  if (active.length === 1 && game.highBidderIndex === active[0]) return beginTrumpChoice(game);
...
function beginTrickPlay(game) {
  ...
  const leader = (game.dealerIndex + 1) % game.playerCount;
  return {
    ...game,
    phase: "playing",
...
export function calculatePinochleMeld(cards, trump) {
  ...
  const items = [];
  const add = (name, points) => { if (points > 0) items.push({ name, points }); };

  for (const rank of [14, 13, 12, 11]) {
    const copies = Math.min(...PINOCHLE_SUITS.map((suit) => count(suit, rank)));
    add(`${pinochleRankLabel(rank)}s around${copies > 1 ? ` ×${copies}` : ""}`, scoreForCopies(AROUND_SCORES[rank], copies));
  }

  const pinochles = Math.min(count("spades", 12), count("diamonds", 11));
  add(`Pinochle${pinochles > 1 ? ` ×${pinochles}` : ""}`, scoreForCopies(PINOCHLE_SCORES, pinochles));

  const runCopies = Math.min(...[14, 10, 13, 12, 11].map((rank) => count(trump, rank)));
  const runScore = [0, 15, 150, 225, 300][Math.min(runCopies, 4)] || 0;
  add(`Run in ${PINOCHLE_SUIT_SYMBOLS[trump]}${runCopies > 1 ? ` ×${runCopies}` : ""}`, runScore);
  ...
  for (const suit of PINOCHLE_SUITS) {
    ...
    add(`${suit === trump ? "Royal" : "Common"} marriage in ${PINOCHLE_SUIT_SYMBOLS[suit]}${extraMarriages > 1 ? ` ×${extraMarriages}` : ""}`, extraMarriages * (suit === trump ? 4 : 2));
  }

  const trumpNines = count(trump, 9);
  add(`Dix${trumpNines > 1 ? ` ×${trumpNines}` : ""}`, trumpNines);
...
function collectPinochleTrick(game) {
  ...
  if (players.every((player) => player.hand.length === 0)) {
    ...
    return finishPinochleRound({ ...game, players, lastTrick, trick: [], trickNumber: game.trickNumber + 1 });
  }
  return {
    ...game,
    players,
    ...
    message: `${game.players[winner.playerIndex].name} took the trick and leads.`,
  };
}
...
export function choosePinochleBotBid(game, playerIndex) {
  ...
  const ceiling = Math.max(game.minimumBid, bestMeld + Math.floor(counters * 0.55));
  const required = game.highBid === null ? game.minimumBid : game.highBid + 1;
