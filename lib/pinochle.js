...
export const shufflePinochleDeck = shuffled;
...
export function createPinochleGame({ playerSeeds, targetScore, startingPlayerIndex = 1 } = {}) {
  ...
  return dealPinochleRound({
    ...
    dealerIndex: (normalizedStartingPlayerIndex - 2 + playerCount) % playerCount,
    roundNumber: 0,
    history: [],
  });
}
...
export function dealPinochleRound(game, random = Math.random) {
  ...
  if (game.playerCount === 2) {
    ...
    return {
      ...
      lastTrick: null,
      roundSummary: null,
      lastWashSummary: null,
      exchangeAcknowledgment: null,
      winnerTeamIds: [],
...
export function passPinochleBid(game, playerIndex) {
  ...
  if (active.length === 1 || biddingCanEndForSameTeam(game, active)) {
...
function advanceBidding(game, afterPlayerIndex) {
  ...
  if ((active.length === 1 && game.highBidderIndex === active[0]) || biddingCanEndForSameTeam(game, active)) {
...
function biddingCanEndForSameTeam(game, activePlayerIndexes) {
  return remainingBiddersAreTeammates(game, activePlayerIndexes)
    && activePlayerIndexes.every((index) => game.players[index].isComputer);
}
...
export function passPinochlePartnerCards(game, playerIndex, cardIds) {
  ...
  const players = game.players.map((player, index) => index === playerIndex
    ? { ...player, hand: player.hand.filter((card) => !cardIds.includes(card.id)) }
    : index === game.highBidderIndex
      ? { ...player, hand: sortPinochleCards([...player.hand, ...selected], game.trump) }
    : player);
  ...
  return {
    ...
    players,
    partnerPasses,
    phase: "acknowledging-exchange",
    currentPlayerIndex: game.highBidderIndex,
    exchangeReturnQueue: remainingPartners.length ? [] : [...game.exchangePartnerIndexes],
    exchangeAcknowledgment: {
      senderIndex: playerIndex,
      recipientIndex: game.highBidderIndex,
      cards: selected,
      nextPhase: remainingPartners.length ? "partner-passing" : "bidder-returning",
      nextPlayerIndex: remainingPartners.length ? remainingPartners[0] : game.highBidderIndex,
    },
    message: `${game.players[game.highBidderIndex].name} received ${game.exchangeCount} cards from ${game.players[playerIndex].name} and must acknowledge them.`,
  };
}
...
export function returnPinochlePartnerCards(game, playerIndex, cardIds) {
  ...
  return {
    ...
    players,
    phase: "acknowledging-exchange",
    currentPlayerIndex: partnerIndex,
    exchangeReturnQueue,
    exchangeAcknowledgment: {
      senderIndex: playerIndex,
      recipientIndex: partnerIndex,
      cards: selected,
      nextPhase: exchangeReturnQueue.length ? "bidder-returning" : "playing",
      nextPlayerIndex: exchangeReturnQueue.length ? playerIndex : game.highBidderIndex,
    },
    message: `${game.players[partnerIndex].name} received ${game.exchangeCount} cards from ${game.players[playerIndex].name} and must acknowledge them.`,
  };
}

export function acknowledgePinochleExchange(game, playerIndex) {
  requirePhaseAndTurn(game, "acknowledging-exchange", playerIndex);
  const acknowledgment = game.exchangeAcknowledgment;
  if (!acknowledgment || acknowledgment.recipientIndex !== playerIndex) {
    throw new Error("Only the player who received the cards can acknowledge them.");
  }
  if (acknowledgment.nextPhase === "partner-passing") {
    return {
      ...game,
      phase: "partner-passing",
      currentPlayerIndex: acknowledgment.nextPlayerIndex,
      exchangeAcknowledgment: null,
      message: `${game.players[acknowledgment.nextPlayerIndex].name} chooses ${game.exchangeCount} cards to send to bidder ${game.players[game.highBidderIndex].name}.`,
    };
  }
  if (acknowledgment.nextPhase === "bidder-returning") {
    const partnerIndex = game.exchangeReturnQueue[0];
    return {
      ...game,
      ...
      currentPlayerIndex: acknowledgment.nextPlayerIndex,
      exchangeAcknowledgment: null,
      message: `${game.players[game.highBidderIndex].name} chooses ${game.exchangeCount} cards to return to ${game.players[partnerIndex].name}.`,
    };
  }
...
function beginTrickPlay(game) {
  ...
  return washImpossiblePinochleContract(readyToPlay);
}

function washImpossiblePinochleContract(game) {
  const maximumTrickPoints = game.players
    .flatMap((player) => player.hand)
    .reduce((sum, card) => sum + ([14, 10, 13].includes(card.rank) ? PINOCHLE_SCORE_SCALE : 0), PINOCHLE_SCORE_SCALE);
  const biddingTeamId = game.players[game.highBidderIndex].teamId;
  const eligibleMeldPoints = game.playerCount === 5
    ? game.melds[game.highBidderIndex].total
    : game.teamMeldPoints[biddingTeamId];
  const maximumContractPoints = eligibleMeldPoints + maximumTrickPoints;
  if (maximumContractPoints >= game.highBid) return game;

  const contractPlayerIndexes = game.playerCount === 5
    ? game.contractPlayerIndexes
    : game.teams[biddingTeamId].memberIds;
  const roundDeltas = game.playerCount === 5
    ? game.melds.map((meld, index) => contractPlayerIndexes.includes(index) ? -game.highBid : meld.total)
    : game.teamMeldPoints.map((points, teamId) => teamId === biddingTeamId ? -game.highBid : points);
  ...
  const washMessage = `${game.players[game.highBidderIndex].name}'s ${game.highBid} bid could reach at most ${maximumContractPoints}. The hand was washed; the bidder team lost ${game.highBid}, and the opponents' meld was scored.`;
  const washSummary = {
    washed: true,
  ...
  const scoredGame = {
    ...
    teams,
    winnerTeamIds,
    roundSummary: washSummary,
    lastWashSummary: washSummary,
    history: [...(game.history || []), washSummary],
  };
  if (winnerTeamIds.length) {
    return { ...scoredGame, phase: "game-over", message: washMessage };
  }
  const redealt = dealPinochleRound(scoredGame);
  return {
    ...redealt,
    lastWashSummary: washSummary,
    message: `${washMessage} ${redealt.message}`,
  };
}
...
function finishFivePlayerPinochleRound(game) {
  ...
  const roundDeltas = scoringPoints.map((points, index) => contractPlayerIndexes.includes(index)
    ? madeContract ? points : -game.highBid
    : points);
  const teams = game.teams.map((team, teamId) => ({ ...team, score: team.score + roundDeltas[teamId] }));
  const topScore = Math.max(...teams.map((team) => team.score));
  const winnerTeamIds = topScore >= game.targetScore ? teams.filter((team) => team.score === topScore).map((team) => team.id) : [];
