...
export const shuffleHeartDeck = shuffled;
...
export function dealHeartsRound(match, random = Math.random) {
  ...
  deck = keepOpeningSafeCardInEveryHand(deck, match.playerCount);
...
function keepOpeningSafeCardInEveryHand(deck, playerCount) {
  const result = [...deck];
  const dealtCardCount = Math.floor(result.length / playerCount) * playerCount;

  for (let playerIndex = 0; playerIndex < playerCount; playerIndex += 1) {
    const playerPositions = result
      .map((card, index) => (index < dealtCardCount && index % playerCount === playerIndex ? index : -1))
      .filter((index) => index >= 0);
    if (playerPositions.some((index) => cardPoints(result[index]) === 0)) continue;

    const donorIndex = result.findIndex((card, index) => {
      if (cardPoints(card) !== 0) return false;
      if (index >= dealtCardCount) return true;
      const donorSeat = index % playerCount;
      if (donorSeat === playerIndex) return false;
      const donorSafeCards = result.filter(
        (candidate, candidateIndex) =>
          candidateIndex < dealtCardCount &&
          candidateIndex % playerCount === donorSeat &&
          cardPoints(candidate) === 0
      ).length;
      return donorSafeCards > 1;
    });
    [result[playerPositions[0]], result[donorIndex]] = [result[donorIndex], result[playerPositions[0]]];
  }
  return result;
}
...
export function getLegalHeartCards(state, playerIndex) {
  ...
  if (state.trick.length) {
    ...
    if (state.trickNumber === 1) {
      return hand.filter((card) => cardPoints(card) === 0);
    }
    return hand;
  }

  if (state.heartsPlayed) return hand;
...
export function chooseBotHeartCard(state, playerIndex) {
  ...
  if (!state.trick.length) {
    ...
    const choices = nonPointCards.length ? nonPointCards : sorted;
    const suitCounts = choices.reduce((counts, card) => ({
      ...counts,
      [card.suit]: (counts[card.suit] || 0) + 1,
    }), {});
    return [...choices].sort((a, b) => (
      suitCounts[a.suit] - suitCounts[b.suit]
      || a.rank - b.rank
    ))[0];
  }
  ...
  const trickPoints = state.trick.reduce((sum, play) => sum + cardPoints(play.card), 0)
    + state.carryoverCards.reduce((sum, card) => sum + cardPoints(card), 0);
