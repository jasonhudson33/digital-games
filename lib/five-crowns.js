...
export function drawCard(game, playerId) {
  if (!canDraw(game, playerId) || !game.deck.length) return game;
  const card = game.deck[0];
  return updatePlayer({ ...game, deck: game.deck.slice(1), turnDrawn: true }, playerId, (player) => ({ ...player, hand: [...player.hand, card] }));
}
...
export function layDownMeld(game, playerId, cardIds) {
  ...
  if (cards.length !== cardIds.length || !isValidMeld(game, cards)) return game;
...
export function discardCard(game, playerId, cardId) {
  ...
  if (isFullyMeldable(next, playerId)) return goOut(next, playerId);
  return advanceAfterDiscard(next, playerId);
}

export function goOut(game, playerId) {
  if (game.outPlayerId || !canAct(game, playerId) || !isFullyMeldable(game, playerId)) return game;
  const player = game.players.find((item) => item.id === playerId);
  const next = withLog({ ...game, outPlayerId: playerId, finalTurnIds: game.players.filter((item) => item.id !== playerId).map((item) => item.id) }, `${player.name} went out! Everyone else gets one final turn.`);
  return advanceAfterDiscard(next, playerId);
}

export function canGoOut(game, playerId) { return canAct(game, playerId) && isFullyMeldable(game, playerId); }
...
export function runComputerStep(game, rng = Math.random) {
  ...
  const shouldTakeDiscard = discard && player.hand.some((card) => card.type === "joker" || (card.type === "normal" && card.rank === discard.rank));
  let next = shouldTakeDiscard ? takeDiscard(game, player.id) : drawCard(game, player.id);
  const available = findValidMelds(next, next.players.find((item) => item.id === player.id)?.hand || []);
  if (available.length) next = layDownMeld(next, player.id, available.sort((a, b) => b.length - a.length)[0].map((card) => card.id));
  const current = next.players.find((item) => item.id === player.id);
  if (canGoOut(next, player.id)) {
    return goOut(next, player.id);
  }
