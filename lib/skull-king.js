...
export function createSkullKingMatch({ playerName = "You", playerCount = 4, random = Math.random } = {}) {
  if (playerCount < 3 || playerCount > 9) {
    throw new Error("Skull King supports three to nine players with the expansion deck.");
  }

  const players = BOT_NAMES.slice(0, playerCount).map((name, index) => ({
    id: index,
...
export function dealSkullKingRound(match, random = Math.random) {
  const deck = shuffleSkullKingDeck(createSkullKingDeck(), random);
  ...
  const players = match.players.map((player, playerIndex) => ({
    ...player,
    hand: sortSkullKingHand(dealt.filter((_, cardIndex) => cardIndex % match.playerCount === playerIndex)),
    bid: null,
    tricks: 0,
    roundBonus: 0,
    wager: 0,
...
export function submitSkullKingBid(state, playerIndex, bid, random = Math.random) {
  if (state.phase !== "bidding" || playerIndex !== 0) return state;
  const safeBid = Math.max(0, Math.min(state.roundNumber, Number(bid)));
  const players = state.players.map((player, index) => ({
    ...player,
    bid: index === playerIndex ? safeBid : chooseBotBid(player.hand, state.roundNumber, random),
  }));
  const leaderIndex = (state.dealerIndex + 1) % state.playerCount;
  return {
    ...state,
    players,
    phase: "playing",
...
export function playSkullKingCard(state, playerIndex, cardId, declaration = null) {
  ...
  if (trick.length < expectedPlays) {
    const nextPlayerIndex = nextActivePlayer(state, playerIndex, state.trickNumber);
    return {
      ...state,
      players,
      trick,
...
export function resolveSkullKingPirateAbility(state, choice = {}, random = Math.random) {
  ...
  if (pending.pirateKey === "mary") {
    ...
    if (!validPlayerIndex(state, targetPlayerIndex) || !players[targetPlayerIndex].hand.length) return state;
...
export function chooseBotPirateAbility(state, random = Math.random) {
  ...
  if (pending.pirateKey === "mary") {
    const candidates = state.players.filter((candidate) => candidate.hand.length);
...
export function collectSkullKingTrick(state) {
  ...
  let leaderIndex = state.lastTrick.nextLeaderIndex;
  if (trickNumber === state.roundNumber && leaderIndex === state.skipFinalTrickPlayerIndex) {
...
function finishSkullKingRound(state) {
  ...
  for (const alliance of state.doubloonAlliances) {
    if (exactBids[alliance.doubloonPlayerIndex] && exactBids[alliance.winnerIndex]) {
  ...
  const points = playersWithBonuses.map((player) => scoreSkullKingRound(player, state.roundNumber));
  ...
  return {
    ...
    roundSummary: {
      ...
      winnerIndexes: gameOver
        ? players.map((player, index) => player.score === highScore ? index : -1).filter((index) => index >= 0)
        : [],
    },
