...
export function createHandFootMatch({ playerName = "You", playerCount = 4, teammateName = "Vega", random = Math.random } = {}) {
  ...
  return createSeatedHandFootMatch(seatedPlayers, random);
}

export function createHandFootRoomMatch({ teamPairs, random = Math.random } = {}) {
...
function createSeatedHandFootMatch(seatedPlayers, random) {
...
export function drawHandFootCards(state, playerIndex, pileIndex = null, cardCount = null) {
  ...
  const cardsDrawnThisTurn = state.cardsDrawnThisTurn || 0;
  const cardsStillNeeded = 2 - cardsDrawnThisTurn;
  if (cardsStillNeeded <= 0) throw new Error("You have already drawn two cards.");
  ...
  const nextCardsDrawn = cardsDrawnThisTurn + drawn.length;
  ...
  return {
    ...
    cardsDrawnThisTurn: nextCardsDrawn,
    turnStage: nextCardsDrawn === 2 ? "play" : "draw",
    message: nextCardsDrawn === 2
...
export function chooseHandFootBotPlay(state, playerIndex) {
  ...
  for (const [rank, group] of Object.entries(groups)) {
    if (team.melds[rank]) {
      options.push(group);
      if (wilds.length) options.push([...group, wilds[0]]);
    } else if (group.length >= 3) {
      options.push(group);
    } else if (group.length === 2 && wilds.length) {
      options.push([...group, wilds[0]]);
    }
  }
  if (wilds.length >= 3 || (team.melds.wild && wilds.length)) options.push(wilds);

  const candidates = (team.opened ? options : combineHandFootMeldOptions(options, cards.length))
    .filter((candidate) => candidate.length && canPlayHandFootCards(state, playerIndex, candidate.map((card) => card.id)))
    .filter((candidate) => team.opened || candidate.reduce((sum, card) => sum + handFootCardPoints(card), 0) >= state.roundRequirement)
    .map((candidate) => ({
      cards: candidate,
      regularWildAdditions: describeHandFootBotWildAdditions(team, candidate),
    }));
  const cleanEndPlayAvailable = candidates.some(({ cards: candidate, regularWildAdditions }) => (
    regularWildAdditions.length === 0 && cards.length - candidate.length <= 1
  ));

  return candidates
    .filter(({ cards: candidate, regularWildAdditions }) => {
      if (regularWildAdditions.length === 0) return true;
      if (cleanEndPlayAvailable || cards.length - candidate.length > 1) return false;
      return regularWildAdditions.every(({ existingLength, combinedLength }) => (
        existingLength < 7 && combinedLength >= 7
      ));
    })
    .sort((left, right) => scoreHandFootBotMeld(right.cards, team) - scoreHandFootBotMeld(left.cards, team))[0]
...
function scoreHandFootBotMeld(cards, team) {
  const additions = buildMeldAdditions(team, cards);
...
function describeHandFootBotWildAdditions(team, cards) {
  const additions = buildMeldAdditions(team, cards);
