...
export function chooseHandFootBotPlay(state, playerIndex) {
  ...
  return candidates
    ...
    .sort((left, right) => scoreHandFootBotMeld(right, team) - scoreHandFootBotMeld(left, team))[0]
    ?.map((card) => card.id) ?? [];
}
...
function scoreHandFootBotMeld(cards, team) {
  const ranks = new Set(cards.filter((card) => !isWildCard(card)).map((card) => String(card.rank)));
  const completedBooks = [...ranks].filter((rank) => (
    (team.melds[rank]?.length || 0) + cards.filter((card) => String(card.rank) === rank || isWildCard(card)).length >= 7
  )).length;
