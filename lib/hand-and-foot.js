...
export function createHandFootRoomMatch({ teamPairs, random = Math.random } = {}) {
  if (!Array.isArray(teamPairs) || teamPairs.length < 2 || teamPairs.length > 4 || teamPairs.some((pair) => !Array.isArray(pair) || pair.length !== 2)) {
    throw new Error("Hand and Foot requires two to four teams of two.");
  }
...
export function playHandFootCards(state, playerIndex, cardIds) {
  ...
  if (updatedPlayer.usingFoot && updatedPlayer.foot.length === 0) {
    ...
    const teammateHasThree = state.viewerPlayerIndex === playerIndex && typeof state.viewerTeammateHasThree === "boolean"
      ...
      : activeCardsFor({ ...state, players }, teammateId).some(isThree);
    if (teammateHasThree) {
