...
import { ensureKillerBunniesCircle } from "./killer-bunnies-circle.js";
...
export async function chooseKillerBunniesRoomEveryoneFeedBunny({ roomCode, token, bunnyId }) {
  return updateGame(roomCode, token, (game, playerIndex) =>
    chooseKillerBunniesEveryoneFeedBunny(game, playerIndex, bunnyId));
}

export async function chooseKillerBunniesRoomBlackCatTarget({ roomCode, token, targetPlayerIndex, bunnyId }) {
  return updateGame(roomCode, token, (game, playerIndex) =>
    chooseKillerBunniesBlackCatTarget(game, playerIndex, Number(targetPlayerIndex), bunnyId));
}

export async function resolveKillerBunniesRoomBlackCatRoll({ roomCode, token }) {
  return updateGame(roomCode, token, (game, playerIndex) => resolveKillerBunniesBlackCatRoll(game, playerIndex));
}

export async function placeKillerBunniesRoomBlackCatClover({ roomCode, token, targetPlayerIndex, bunnyId }) {
  return updateGame(roomCode, token, (game, playerIndex) =>
    placeKillerBunniesBlackCatClover(game, playerIndex, Number(targetPlayerIndex), bunnyId));
}

export async function discardKillerBunniesRoomBlackCatClover({ roomCode, token }) {
  return updateGame(roomCode, token, (game, playerIndex) => discardKillerBunniesBlackCatClover(game, playerIndex));
}

export async function chooseKillerBunniesRoomAuctionTarget({ roomCode, token, targetPlayerIndex, bunnyId }) {
  return updateGame(roomCode, token, (game, playerIndex) =>
    chooseKillerBunniesAuctionTarget(game, playerIndex, Number(targetPlayerIndex), bunnyId));
}

export async function placeKillerBunniesRoomAuctionBid({ roomCode, token, amount }) {
  return updateGame(roomCode, token, (game, playerIndex) =>
    placeKillerBunniesAuctionBid(game, playerIndex, amount === null || amount === "pass" ? null : Number(amount)));
}

export async function chooseKillerBunniesRoomBunnyExchangeGive({ roomCode, token, bunnyId }) {
  return updateGame(roomCode, token, (game, playerIndex) =>
    chooseKillerBunniesBunnyExchangeGive(game, playerIndex, bunnyId));
}

export async function resolveKillerBunniesRoomBunnyExchange({ roomCode, token, targetPlayerIndex, bunnyIds }) {
  return updateGame(roomCode, token, (game, playerIndex) =>
    resolveKillerBunniesBunnyExchange(game, playerIndex, Number(targetPlayerIndex), bunnyIds));
}
...
export async function resolveKillerBunniesRoomWeaponReuse({ roomCode, token, choice }) {
  return updateGame(roomCode, token, (game, playerIndex) =>
    resolveKillerBunniesWeaponReuse(game, playerIndex, choice));
}

export async function resolveKillerBunniesRoomAreaWeaponRoll({ roomCode, token }) {
  return updateGame(roomCode, token, (game, playerIndex) => resolveKillerBunniesAreaWeaponRoll(game, playerIndex));
}

export async function chooseKillerBunniesRoomBlueRollTarget({ roomCode, token, targetPlayerIndex, bunnyId }) {
  return updateGame(roomCode, token, (game, playerIndex) =>
    chooseKillerBunniesBlueRollTarget(game, playerIndex, Number(targetPlayerIndex), bunnyId));
}

export async function resolveKillerBunniesRoomBlueCardRoll({ roomCode, token }) {
  return updateGame(roomCode, token, (game, playerIndex) => resolveKillerBunniesBlueCardRoll(game, playerIndex));
}

export async function chooseKillerBunniesRoomNumber({ roomCode, token, value }) {
  return updateGame(roomCode, token, (game, playerIndex) => chooseKillerBunniesNumber(game, playerIndex, value));
}

export async function resolveKillerBunniesRoomBlueSpecialRoll({ roomCode, token }) {
  return updateGame(roomCode, token, (game, playerIndex) => resolveKillerBunniesBlueSpecialRoll(game, playerIndex));
}

export async function resolveKillerBunniesRoomCardDiceRoll({ roomCode, token, choiceId }) {
  return updateGame(roomCode, token, (game, playerIndex) =>
    resolveKillerBunniesCardDiceRoll(game, playerIndex, choiceId));
}

export async function resolveKillerBunniesRoomCardAction({ roomCode, token, cardAction }) {
  return updateGame(roomCode, token, (game, playerIndex) =>
    resolveKillerBunniesCardAction(game, playerIndex, cardAction || {}));
}

export async function donateKillerBunniesRoomBounty({ roomCode, token, bunnyId, amount }) {
  return updateGame(roomCode, token, (game, playerIndex) =>
    donateKillerBunniesBounty(game, playerIndex, bunnyId, Number(amount)));
}

export async function chooseKillerBunniesRoomRevivedBunny({ roomCode, token, bunnyId }) {
  return updateGame(roomCode, token, (game, playerIndex) => chooseKillerBunniesRevivedBunny(game, playerIndex, bunnyId));
}

export async function chooseKillerBunniesRoomRoamingTarget({ roomCode, token, targetPlayerIndex, bunnyId }) {
  return updateGame(roomCode, token, (game, playerIndex) =>
    chooseKillerBunniesRoamingTarget(game, playerIndex, Number(targetPlayerIndex), bunnyId));
}

export async function resolveKillerBunniesRoomRoamingRoll({ roomCode, token }) {
  return updateGame(roomCode, token, (game, playerIndex) => resolveKillerBunniesRoamingRoll(game, playerIndex));
}
...
export async function chooseKillerBunniesRoomMisfortuneTarget({ roomCode, token, bunnyId, targetPlayerIndex }) {
  return updateGame(roomCode, token, (game, playerIndex) =>
    chooseKillerBunniesMisfortuneTarget(game, playerIndex, bunnyId, Number.isInteger(Number(targetPlayerIndex)) ? Number(targetPlayerIndex) : playerIndex));
}
...
export async function chooseKillerBunniesRoomPlayerTarget({ roomCode, token, targetPlayerIndex }) {
  return updateGame(roomCode, token, (game, playerIndex) =>
    chooseKillerBunniesPlayerTarget(game, playerIndex, Number(targetPlayerIndex)));
}

export async function chooseKillerBunniesRoomUtilityBunnyTarget({ roomCode, token, targetPlayerIndex, bunnyId }) {
  return updateGame(roomCode, token, (game, playerIndex) =>
    chooseKillerBunniesUtilityBunnyTarget(game, playerIndex, Number(targetPlayerIndex), bunnyId));
}
...
export async function callKillerBunniesRoomPovertyPoker({ roomCode, token, stakes }) {
  return updateGame(roomCode, token, (game, playerIndex) =>
    callKillerBunniesPovertyPoker(game, playerIndex, stakes));
}

export async function anteKillerBunniesRoomPovertyPoker({ roomCode, token, selections }) {
  return updateGame(roomCode, token, (game, playerIndex) =>
    anteKillerBunniesPovertyPoker(game, playerIndex, selections));
}

export async function resolveKillerBunniesRoomPovertyPokerRoll({ roomCode, token, choice }) {
  return updateGame(roomCode, token, (game, playerIndex) =>
    resolveKillerBunniesPovertyPokerRoll(game, playerIndex, choice));
}
...
function serializeRoom(room, token) {
  ...
  if (game.pendingAction?.effect === "timidReroll") {
    delete game.pendingAction.originalState;
    delete game.pendingAction.acceptedState;
  }
  const viewerControlsAction = game.pendingAction?.playerIndex === viewerPlayerIndex;
  const programmedRunsArePublic = Boolean(game.selectiveDisclosureBy?.length);
  const visibleActionHand = (player, index) => {
    if (index === viewerPlayerIndex) return player.hand;
    if (!viewerControlsAction) return [];
    if (game.phase === "weaponExchange") return player.hand.filter((card) => card.kind === "weapon");
    if (game.phase === "showBunnyExchange") {
      return player.hand.filter((card) => game.pendingAction.revealedBunnyIds?.includes(card.id));
    }
    return [];
  };
  game.players = game.players.map((player, index) => ({
    ...player,
    insightCarrots: index === viewerPlayerIndex ? player.insightCarrots || [] : [],
    savedSpecials: player.savedSpecials || [],
    feedingObligations: player.feedingObligations || [],
    handCount: player.hand.length,
    hand: visibleActionHand(player, index),
    topRun: index === viewerPlayerIndex || programmedRunsArePublic ? player.topRun : player.topRun ? { hidden: true, id: `hidden-top-${index}` } : null,
    bottomRun: index === viewerPlayerIndex || programmedRunsArePublic ? player.bottomRun : player.bottomRun ? { hidden: true, id: `hidden-bottom-${index}` } : null,
