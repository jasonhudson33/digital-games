...
function validatePlayers(players) {
  ...
  if ((players.length !== 1 && players.length < SevenUpGame.MIN_PLAYERS) || players.length > SevenUpGame.MAX_PLAYERS) {
    throw new Error(
...
export async function joinRoom({ roomCode, seatId, name }) {
  ...
  const seat = seatId ? requireSeat(room, seatId) : {
    seatId: `seat-${room.players.length + 1}`,
    label: `Player ${room.players.length + 1}`,
    displayName: "",
    playerType: "human",
    token: null,
    claimed: false,
  ...
  if (!seatId) {
    if (room.players.length >= SevenUpGame.MAX_PLAYERS) {
...
export async function addComputerPlayer({ roomCode, token }) {
  const room = await requireRoom(roomCode);
  requireHost(room, token);
  if (room.status !== "waiting") throw new Error("This room has already started.");
  if (room.players.length >= SevenUpGame.MAX_PLAYERS) throw new Error("This room is full.");
  const usedSeatNumbers = new Set(room.players.map((player) => Number(player.seatId.replace("seat-", ""))));
  let seatNumber = 1;
  while (usedSeatNumbers.has(seatNumber)) seatNumber += 1;
  room.players.push({
    seatId: `seat-${seatNumber}`,
    label: `Computer ${seatNumber}`,
    displayName: `Computer ${seatNumber}`,
    playerType: "computer",
    token: null,
    claimed: true,
  });
...
export async function removeComputerPlayer({ roomCode, token, seatId }) {
  const room = await requireRoom(roomCode);
  requireHost(room, token);
  if (room.status !== "waiting") throw new Error("This room has already started.");
  const index = room.players.findIndex((player) => player.seatId === seatId);
  if (index < 0 || room.players[index].playerType !== "computer") {
    throw new Error("Only computer players can be removed.");
  }
...
export async function startRoom({ roomCode, token }) {
  ...
  if (room.players.length < SevenUpGame.MIN_PLAYERS) {
    throw new Error(`7-Up needs at least ${SevenUpGame.MIN_PLAYERS} players to start.`);
  }
...
function requireHost(room, token) {
  if (room.hostToken !== token) throw new Error("Only the host can change the room.");
}
