...
export async function createRoom({ dealerIndex, players, name }) {
  const roomCode = await uniqueRoomCode();
  const hostToken = randomToken(24);
  let hostSeatId = "";
  const roomPlayers = players.map((player, index) => {
    const seatId = `seat-${index + 1}`;
    const defaultLabel = (player.name || `Player ${index + 1}`).trim();
    const roomPlayer = {
      seatId,
      label: defaultLabel,
    ...
    if (player.playerType === "human" && !hostSeatId) {
      ...
      roomPlayer.displayName = name?.trim() || defaultLabel;
      hostSeatId = seatId;
    }
    return roomPlayer;
  });
