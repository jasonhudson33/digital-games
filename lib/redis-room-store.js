  ...
  return {
    async save(room) {
      ...
      if (process.env.REDIS_URL) {
        await (await getRedisClient(redisClientFactory)).set(key, payload, { EX: ttlSeconds });
        return;
      }
    ...
    async load(roomCode) {
      ...
      const payload = process.env.REDIS_URL
        ? await (await getRedisClient(redisClientFactory)).get(key)
        : rooms.get(key);
...
async function getRedisClient(redisClientFactory) {
  if (!redisClientState.promise) {
    ...
    redisClientState.promise = client.connect()
      .then(() => client)
      .catch(() => {
        redisClientState.promise = null;
        throw new Error("Could not connect to room storage. Check the production REDIS_URL and try again.");
      });
  }
