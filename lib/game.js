...
export class SevenUpGame {
  ...
  constructor(players, dealerIndex = 0, seed = Math.random()) {
    ...
    // playerStates is keyed by name, so two players sharing one collapse into a
    // single entry: `players` keeps N seats while `playerStates` has N-1, the
    // deal loop pushes both seats' cards into the shared entry, and each of them
    // reads the other's hand. That used to happen silently — a room where two
    // people typed "Sam" dealt 35 cards to each of them out of a 52-card deck.
    // Fail loudly instead; lib/rooms.js guarantees uniqueness upstream.
    if (new Set(players).size !== players.length) {
      throw new Error("Every player needs a distinct name.");
    }
  ...
  static fromSnapshot(snapshot) {
    // Rooms persisted before the constructor guard could hold a corrupted
    // snapshot. Surface it rather than restoring the shared-hand state.
    if (new Set(snapshot.players).size !== snapshot.players.length) {
      throw new Error("This room was dealt with duplicate player names and cannot be restored.");
    }
...
export function chooseComputerMove(legalMoves, hand = legalMoves) {
  ...
  return [...legalMoves].sort((left, right) => (
    computerMoveScore(right, handKeys, suitCounts) - computerMoveScore(left, handKeys, suitCounts)
    || compareCards(left, right)
  ))[0];
}

function computerMoveScore(card, handKeys, suitCounts) {
  if (handKeys.size === 1) return 100_000;
  const directions = card.rank === 7 ? [-1, 1] : [card.rank < 7 ? -1 : 1];
  const chainLength = directions.reduce((total, direction) => {
    let rank = card.rank + direction;
    let length = 0;
    while (rank >= 1 && rank <= 13 && handKeys.has(`${card.suit}:${rank}`)) {
      length += 1;
      rank += direction;
    }
    return total + length;
  }, 0);
  return (card.rank === 7 ? 10_000 : 0)
    + chainLength * 100
    + (suitCounts[card.suit] || 0) * 10
    + Math.abs(card.rank - 7);
}
