...
/**
 * Turns of nothing happening before the round is called a draw.
 *
 * BANG! has no written draw, and it did not need one at the table because the
 * players get bored first. Here the board can reach a position nobody can break
 * — the last Outlaw holding four BANG! cards at weapon range 1 while every
 * survivor sits at distance 2 behind a Mustang — and then the game genuinely
 * never ends. Left alone it drew and discarded through 400,000 computer steps
 * without a single life changing hands, and because the host's client re-fires
 * on every state change, that is a full-state PUT to the room API every 560 ms,
 * for as long as the tab stays open.
 *
 * The bound is measured, not guessed: across 100 seeded computer games the
 * longest quiet stretch inside a game that *did* finish was 21 turns, and the
 * longest complete game was 79 turns. 60 leaves roughly a threefold margin.
 */
export const STALEMATE_TURNS = 60;

/** Lives and survivors — the only things whose change counts as progress. */
function progressMark(game) {
  return game.players.reduce(
    (mark, player) => (player.alive ? `${mark}|${player.id}:${player.lives}` : mark),
    "",
  );
}
...
function beginNextTurn(game, rng) {
  ...
  // `?? ` rather than a bare read: rooms persisted before this rule existed
  // have neither field, and they should pick it up rather than break.
  const mark = progressMark(next);
  const quietTurns = mark === (game.lastProgressMark ?? progressMark(game)) ? (game.quietTurns ?? 0) + 1 : 0;
  next = { ...next, quietTurns, lastProgressMark: mark };
  if (quietTurns >= STALEMATE_TURNS) return declareStalemate(next);

  return resolveStartOfTurn(next, rng);
}

function declareStalemate(game) {
  const alive = livingPlayers(game);
  return {
    ...game,
    phase: "finished",
    ...
    winner: "draw",
    log: [
      `No shot has landed in ${STALEMATE_TURNS} turns. The standoff ends in a draw between ${alive.map((player) => player.name).join(", ")}.`,
      ...game.log,
    ].slice(0, 60),
  };
}
