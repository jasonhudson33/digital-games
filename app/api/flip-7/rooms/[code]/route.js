...
export async function GET(_request, context) {
  try {
    ...
    const state = await flip7RoomStore.load(normalizeCode(code));
...
export async function PUT(request, context) {
  try {
    ...
    if (!body?.state || normalizeCode(body.state.roomCode) !== roomCode) {
      return Response.json({ error: "Invalid Flip 7 room state." }, { status: 400 });
    }
    const current = await flip7RoomStore.load(roomCode);
    ...
    await flip7RoomStore.save(body.state);
