...
export async function GET(_request, context) {
  try {
    ...
    const state = await ticketToRideRoomStore.load(normalizeCode(code));
...
export async function PUT(request, context) {
  try {
    ...
    if (!body?.state || normalizeCode(body.state.roomCode) !== roomCode) {
      return Response.json({ error: "Invalid Ticket to Ride room state." }, { status: 400 });
    }
    const current = await ticketToRideRoomStore.load(roomCode);
    ...
    await ticketToRideRoomStore.save(body.state);
