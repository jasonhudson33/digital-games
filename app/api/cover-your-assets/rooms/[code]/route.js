...
export async function GET(_request, context) {
  try {
    ...
    const state = await coverYourAssetsRoomStore.load(normalizeCode(code));
...
export async function PUT(request, context) {
  try {
    ...
    if (!body?.state || normalizeCode(body.state.roomCode) !== roomCode) {
      return Response.json({ error: "Invalid Cover Your Assets room state." }, { status: 400 });
    }
    const current = await coverYourAssetsRoomStore.load(roomCode);
    ...
    await coverYourAssetsRoomStore.save(body.state);
