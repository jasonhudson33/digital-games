...
export async function GET(_request, context) {
  try {
    ...
    const state = await dominionRoomStore.load(normalizeCode(code));
...
export async function PUT(request, context) {
  try {
    ...
    if (!body?.state || body.state.game !== "dominion" || normalizeCode(body.state.roomCode) !== roomCode) {
      return Response.json({ error: "Invalid Dominion room state." }, { status: 400 });
    }
    const current = await dominionRoomStore.load(roomCode);
    ...
    await dominionRoomStore.save(body.state);
