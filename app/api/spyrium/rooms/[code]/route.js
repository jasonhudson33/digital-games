...
export async function GET(_request, context) {
  try {
    ...
    const state = await spyriumRoomStore.load(normalizeCode(code));
...
export async function PUT(request, context) {
  try {
    ...
    if (!body?.state || body.state.game !== "spyrium" || normalizeCode(body.state.roomCode) !== roomCode) {
      return Response.json({ error: "Invalid Spyrium room state." }, { status: 400 });
    }
    const current = await spyriumRoomStore.load(roomCode);
    ...
    await spyriumRoomStore.save(body.state);
