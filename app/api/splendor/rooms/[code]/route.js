...
export async function GET(_request, context) {
  try {
    ...
    const state = await splendorRoomStore.load(normalizeCode(code));
...
export async function PUT(request, context) {
  try {
    ...
    if (!body?.state || body.state.game !== "splendor" || normalizeCode(body.state.roomCode) !== roomCode) {
      return Response.json({ error: "Invalid Splendor room state." }, { status: 400 });
    }
    const current = await splendorRoomStore.load(roomCode);
    ...
    await splendorRoomStore.save(body.state);
