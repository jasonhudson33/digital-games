...
export async function GET(_request, context) {
  try {
    ...
    const state = await sequenceRoomStore.load(normalizeCode(code));
...
export async function PUT(request, context) {
  try {
    ...
    if (!body?.state || normalizeCode(body.state.roomCode) !== roomCode) {
      return Response.json({ error: "Invalid Sequence room state." }, { status: 400 });
    }
    const current = await sequenceRoomStore.load(roomCode);
    ...
    await sequenceRoomStore.save(body.state);
